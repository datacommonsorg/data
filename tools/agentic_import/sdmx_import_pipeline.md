...
-   **Gemini CLI Errors**: If the schema mapping step fails, check the Gemini CLI logs in `.datacommons/runs/<gemini_run_id>/` (includes `gemini_cli.log` and `processor.log`). Refer to the [main README](./README.md#debugging) for detailed debugging instructions.
