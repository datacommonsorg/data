...
class StatVarsMap:
  ...
  """
    ...
    """
        ...
        prefix = ''
        value = mcf_file_util.strip_namespace(value)
    ...
    """
        ...
        if not dcid:
            ...
            for p in self._config.get('default_statvar_pvs', {}).keys():
                if p in props and p not in dcid_ignore_props:
                    props.remove(p)
                    value = mcf_file_util.strip_namespace(pvs[p])
        ...
        if remap_dcid:
            ...
            dcid = remap_dcid
        pvs['Node'] = mcf_file_util.add_namespace(dcid)
        logging.level_debug() and logging.log(
    ...
    """
        ...
        for namespace, pv_map in pv_map_dict.items():
            for key, pvs in pv_map.items():
                ...
                if dcid:
                    dcid = mcf_file_util.strip_namespace(dcid)
    ...
    """
        ...
        if schemaless_props:
            ...
            pvs['measuredProperty'] = mcf_file_util.add_namespace(dcid)
        logging.level_debug() and logging.log(
    ...
    def format_svobs(self, svobs: dict) -> dict:
        ...
        for prop, value in svobs.items():
            ...
            numeric_value = mcf_file_util.get_numeric_value(value)
            if numeric_value:
    ...
    def _load_existing_statvars(self, mcf_file: list) -> dict:
        fp_nodes = {}
        if mcf_file:
            statvar_nodes = mcf_file_util.load_mcf_nodes(mcf_file)
