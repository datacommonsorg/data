...
class TestValidationRunner(unittest.TestCase):
    ...
    @patch('tools.import_validation.runner.Validator')
    def test_runner_resolves_goldens_paths_relative_to_config(self, MockValidator):
        # Setup mock
        mock_validator_instance = MockValidator.return_value
        mock_validator_instance.validate_goldens.return_value = ValidationResult(
            ValidationStatus.PASSED, 'GOLDENS_CHECK')

        # Create a config that has a GOLDENS_CHECK rule with relative paths
        with open(self.config_path, 'w') as f:
            json.dump(
                {
                    'rules': [{
                        'rule_id': 'check_goldens_relative',
                        'validator': 'GOLDENS_CHECK',
                        'params': {
                            'golden_files': 'golden_data/golden_summary_report.csv',
                            'input_files': 'some_input.csv'
                        }
                    }]
                }, f)
        pd.DataFrame({'StatVar': ['sv1']}).to_csv(self.stats_path, index=False)
        ...
        # Check that the validator was called with rule_params containing resolved absolute paths
        mock_validator_instance.validate_goldens.assert_called_once()
        call_args, _ = mock_validator_instance.validate_goldens.call_args
        # The first arg is df, second is rule_params
        rule_params = call_args[1]

        expected_golden_path = os.path.join(os.path.dirname(os.path.abspath(self.config_path)), 'golden_data/golden_summary_report.csv')
        expected_input_path = os.path.join(os.path.dirname(os.path.abspath(self.config_path)), 'some_input.csv')

        self.assertEqual(rule_params['golden_files'], expected_golden_path)
        self.assertEqual(rule_params['input_files'], expected_input_path)
