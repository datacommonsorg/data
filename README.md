...
A common practice is to add the project root (`.`) to the `PYTHONPATH`. You may also need to add other directories that contain modules imported directly by name (e.g. `import my_module` instead of `from my_package import my_module`).
