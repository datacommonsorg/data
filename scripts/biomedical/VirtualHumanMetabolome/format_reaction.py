...
def main():
    ...
    df_map = pd.read_csv(file_gemid, sep = '\t')
    ...
    ## Use abbreviations to generate dcids where humanGEMids are missing
    for p in range(len(list_null)):
            
            l = list_null[p]
            df_rxn.loc[l,"Id"] = "bio/" + df_rxn.loc[l,"abbreviation"]
    
    df_rxn.update('"' + df_rxn[['description', 'formula', 'ecnumber']].astype(str) + '"')

    df_rxn.to_csv(file_output, index = None)
