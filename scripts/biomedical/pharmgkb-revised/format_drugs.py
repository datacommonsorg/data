...
def format_str_cols(df):
    ...
    df['ATC'] = df['ATC'].str.split(' ').str[0]
    df['MeSH'] = df['MeSH'].str.split('(').str[0]
    df['MeSH'] = df['MeSH'].str.split(' ').str[0]
    return df
...
def format_enum_cols(df):
    ...
    df['dosingGuidelineSource'] = df['dosingGuidelineSource'].str.replace('"', '')
...
def format_dcid(df):
    df = df.assign(dcid=np.where(df['PubChem Compound Identifiers'].notnull(), 'CID' + df['PubChem Compound Identifiers'], df['ATC']))
    df = df.assign(dcid=np.where(df['dcid'].notnull(), df['dcid'], df['MeSH']))
    df['nameId'] = df['Name'].replace('-', '_')
    df = df.assign(dcid=np.where(df['dcid'].notnull(), df['dcid'], df['nameId']))
...
def wrapper_fun(df):
    ...
    df.update('"' +
       ...
       'PubChemSubstance', 'IUPHARLigand', 'PDB',
       'CanadianDrugsProductDatabase', 'ClinicalTrialsgov',
       'FDADrugLabelatDailyMed', 'KEGGDrug', 'NationalDrugCodeDirectory',
       'TherapeuticTargetsDatabase', 'BindingDB', 'HET', 'HMDB', 'GenBank',
       'UniProtKB', 'ATCC', 'UMLS', 'NDFRT', 'MeSH', 'pgkbTags']].astype(str) + '"')
