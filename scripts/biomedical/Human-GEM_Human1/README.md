...
The recon-store-metabolites-1.tsv file can be downloaded from [Virtual Metabolic Human database](https://www.vmh.life/#human/all): Choose "4138 metabolites found", set page site to 4138 and download the file. The hmdb.csv file was sparsed from xml file downloaded from the "All Metabolites" dataset which can be found [here]((https://hmdb.ca/downloads)
...
recon-store-metabolites-1.tsv and hmdb.csv are to combined with human1 metabolites.tsv to generate mapping between humanGEMID and humanMetabolomeDatabaseID.
...
First, you need to genertate a csv file with mapping between humanGEMID and humanMetabolomeDatabaseID, which is required to format human1 metabolites.tsv dataset
...
You can run all the scripts at once by execute the "format_all.sh" file  
