...
[`uniprot_id_mapping_test.csv`](uniprot_id_mapping_test.csv) subsetted only the UniProt names needed for the [`actions_test.tsv`](actions_test.tsv) and [`protein_drug_interactions_test.tsv`](protein_drug_interactions_test.tsv)
[`archived_accessions_test.csv`](archived_accessions_test.csv) subsetted only the UniProt accession IDs needed for the [`actions_test.tsv`](actions_test.tsv) and [`protein_drug_interactions_test.tsv`](protein_drug_interactions_test.tsv)
[`archived_emsembls_test.csv`](archived_emsembls_test.csv) subsetted only the Ensembl Protein IDs contained in [`actions_test.tsv`](actions_test.tsv) and [`protein_drug_interactions_test.tsv`](test-data/protein_drug_interactions_test.tsv)
