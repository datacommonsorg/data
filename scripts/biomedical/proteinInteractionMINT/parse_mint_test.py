...
CONST_PSIMI_TO_DCID = {
    'MI:0084': 'PhageDisplay',
    'MI:0915': 'PhysicalAssociation',
    'MI:0471': 'Mint',
    'MI:0676': 'TandemAffinityPurification'
}
    ...
    def test_main(self):
        ...
        for line in lines:
            if not line:
                continue
            term = line.split('\t')
            schema, new_source_map = parse_mint.get_schema_from_text(
                term, new_source_map, psimi_to_dcid)
            if schema:
