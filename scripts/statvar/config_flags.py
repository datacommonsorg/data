...
flags.DEFINE_string(
    'shard_input_by_column', '', 'Shard input data by unique values in column.'
)
...
flags.DEFINE_list(
    'pv_map', [], 'Comma separated list of namespace:file with property values.'
)
flags.DEFINE_list(
    'input_data', [], 'Comma separated list of data files to be processed.'
)
...
flags.DEFINE_integer(
    'input_rows', sys.maxsize, 'Number of rows per input file to process.'
)
flags.DEFINE_integer(
    'input_columns', sys.maxsize, 'Number of columns in input file to process.'
)
flags.DEFINE_integer(
...
flags.DEFINE_string(
    'output_path', '', 'File prefix for output mcf, csv and tmcf.'
)
flags.DEFINE_string(
...
flags.DEFINE_integer(
    'log_level', logging.INFO, 'Log level messages to be shown.'
)

# Flags for place name resolution
flags.DEFINE_string(
    'dc_api_key', '', 'DataCommons v2 API key used for APIs such as v2/resolve'
)
flags.DEFINE_string(
    'maps_api_key', '', 'Maps API key for place lookup by name.'
)
flags.DEFINE_list(
    'places_csv', [], 'CSV file with place names and dcids to match.'
)
flags.DEFINE_string(
    'places_resolved_csv',
    '',
...
flags.DEFINE_list(
    'places_within', [], 'List of places types for name reoslution.'
)
flags.DEFINE_string(
...
_DEFAULT_CONFIG = {
    ...
    'ignore_numeric_commas': True,  # Numbers may have commas
    'input_reference_column': '#input',
    'input_min_columns_per_row': 2,
    'input_data': _FLAGS.input_data,
    'input_encoding': _FLAGS.input_encoding,
    'input_xls': _FLAGS.input_xls,
    'pv_map_drop_undefined_nodes': (
        False
    ),  # Don't drop undefined PVs in the column PV Map.
    'duplicate_svobs_key': '#ErrorDuplicateSVObs',
    'duplicate_statvars_key': '#ErrorDuplicateStatVar',
    'drop_statvars_without_svobs': 1,
    ...
    'aggregate_duplicate_svobs': _FLAGS.aggregate_duplicate_svobs,
    'merged_pvs_property': '#MergedSVObs',
    ...
    # statvar dcid.
    'schemaless': _FLAGS.schemaless,
    # Whether to lookup DC API and drop undefined PVs in statvars.
    'schemaless_statvar_comment_undefined_pvs': False,
    ...
    'existing_statvar_mcf': _FLAGS.existing_statvar_mcf,
    'existing_schema_mcf': _FLAGS.existing_schema_mcf,
    ...
    'statvar_dcid_remap_csv': _FLAGS.statvar_dcid_remap_csv,
    ...
    'use_all_numeric_data_values': False,
    # Number format in input.
    'number_decimal': '.',  # decimal character
    'number_separator': ', ',  # separators stripped.
    ...
    # Output options
    'generate_statvar_mcf': True,  # Generate MCF file with all statvars
    'generate_csv': True,  # Generate CSV with SVObs
    'output_csv_mode': 'w',  # Overwrite output CSV file.
    'output_columns': [],  # Emit all SVObs PVs into output csv
    'generate_tmcf': True,  # Generate tMCF for CSV columns
    'skip_constant_csv_columns': (
        True
    ),  # Skip emitting columns with constant values in the csv
    'output_only_new_statvars': True,  # Drop existing statvars from output
    ...
    'dc_api_batch_size': 100,
    # Settings from flags
    'pv_map': _FLAGS.pv_map,
    'input_rows': _FLAGS.input_rows,
    'input_columns': _FLAGS.input_columns,
