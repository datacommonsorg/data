...
class TestMCFFileUtil(unittest.TestCase):
  ...
  def test_strip_namespace(self):
    self.assertEqual(
        mcf_file_util.strip_namespace('Count_Person'), 'Count_Person'
    )
    self.assertEqual(
        mcf_file_util.strip_namespace('dcs:Count_Person'), 'Count_Person'
    )
    self.assertEqual(
        mcf_file_util.strip_namespace('dcid:Count_Person'), 'Count_Person'
    )
    self.assertEqual(mcf_file_util.strip_namespace('"abc:123"'), '"abc:123"')
    self.assertEqual(mcf_file_util.strip_namespace(10), 10)

  def test_add_namespace(self):
    self.assertEqual(
        mcf_file_util.add_namespace('Count_Person'), 'dcid:Count_Person'
    )
    self.assertEqual(
        mcf_file_util.add_namespace('dcs:Count_Person'), 'dcs:Count_Person'
    )
  ...
  def test_normalize_range(self):
    self.assertEqual(
        '[10 20 Years]', mcf_file_util.normalize_value('[ 10 20 Years ]')
    )
    self.assertEqual(
        'dcid:Years10To20',
    ...
    self.assertEqual(
        '[- 20 Years]', mcf_file_util.normalize_value('[ - 20 Years ]')
    )
    self.assertEqual(
        'dcid:YearsUpto20',
    ...
    self.assertEqual(
        'dcid:Years20', mcf_file_util.normalize_value('[ 20  Years ]', True)
    )
    self.assertEqual(
        'dcid:Years20Onwards',
        mcf_file_util.normalize_value('[ 20 - Years ]', True),
    ...
    self.assertEqual(
        'dcid:Upto20', mcf_file_util.normalize_value('[ - 20 ]', True)
    )

  def test_load_mcf_file(self):
    mcf_nodes = mcf_file_util.load_mcf_nodes(
        os.path.join(_module_dir_, 'test_data', 'sample_output_stat_vars.mcf')
    )
    ...
    for dcid, node in mcf_nodes.items():
      ...
      self.assertEqual(dcid, node['Node'])
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
      ...
      diff_str = diff_mcf_nodes({dcid: expected_node}, {dcid: mcf_nodes[dcid]})
