  ...
  if url_is_gdrive_folder(input_drive_path):
    ...
    while True:
      ...
      for item in items:
        ...
        drive_gsheet_url = (
            'https://docs.google.com/spreadsheets/d/' + item['id']
        )
        logging.info(
            'Processing sheet : {0} - ({1})'.format(
    ...
    with multiprocessing.Pool(parallelism) as pool:
      for index, data in data_sets.items():
        ...
        metadata_sheet = data.get(
            'metadata', data_sets.get('', {}).get('metadata', '')
        )
        if metadata_sheet:
        ...
        if output_path:
          ...
          data_config['output_path'] = (
              output_path + '-' + gs.title.replace(' ', '_') + str(index)
          )
        else:
          ...
          if 'processedcsv' not in data_sets:
            ...
            if not ws:
              logging.fatal(f'Unable to add worksheet: processed_csv{index}')
        ...
        if parallelism > 1:
          task = pool.apply_async(stat_var_processor.process, kwds=process_args)
        else:
