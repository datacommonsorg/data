...
class PropertyValueCacheTest(unittest.TestCase):
  ...
  def test_add_entry(self):
    ...
    pv_cache.add({'dcid': 'geoId/06', 'typeOf': 'State'})
    pv_cache.add(
        {'dcid': 'country/IND', 'placeId': 'ChIJkbeSa_BfYzARphNChaFPjNc'}
    )
    ...
    self.assertEqual(
        expected_entry1, pv_cache.get_entry(prop='name', value='California')
    )
    self.assertEqual(expected_entry1, pv_cache.get_entry('geoId/06', 'dcid'))
    ...
    self.assertEqual(expected_entry2, pv_cache.get_entry('country/IND', 'dcid'))
  ...
  def test_flatten_dict(self):
    ...
    self.assertEqual(
        [
            ...
            {'name': 'CA', 'dcid': 'geoId/06', 'typeOf': 'AdministrativeArea1'},
            {'name': 'California', 'dcid': 'geoId/06', 'typeOf': 'State'},
            {'name': 'CA', 'dcid': 'geoId/06', 'typeOf': 'State'},
        ],
        name_type_pvs,
    )
