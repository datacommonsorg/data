...
To create a web form for any script with commandline args, use it as follows:
  ...
  its stdout/strerr is returned:
    python3 my_process.py --my_flag1=123 --my_flag2=def --input_file=<form-value>
'''
...
class ProcessHandler(server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        end_msg = f'Completed script: "{cmd}", Return code: {return_code}, time: {end_time - start_time:.3f} secs.\n'
...
def run_http_server(http_port: int = 0,
                    script: str = '',
                    module: str = '__main__',
                    config: dict = {}):
  ...
  '''
    ...
    httpd = ThreadedHTTPServer(http_port=config.get('http_port',
                                                    _FLAGS.http_port),
