...
flags.DEFINE_string(
    'http_config', '', 'File with configurations for the HTTP server.'
)
flags.DEFINE_integer(
    'http_server_timeout',
    86400 * 365,
...
_HTTP_CONFIG = {
    # List of Forms
    'script': '',  # Python script to be invoked when form is submitted.
    'forms': [
        # Form settings for one form.
        {
            'form_title': 'SampleForm',  # Title for the form
            'form_input': [
                # List of form inputs
                {
                    'description': (
...
class ThreadedHTTPServer:
  ...
  def serve_forever(self, timeout: int = sys.maxsize):
    ...
    for i in range(self._num_threads):
      ...
      thread = ThreadedHTTPListner(self._sock, i, self._addr, self._handler)
