...
The values are stored as a dict with any selected property such as dcid as the key.
The cache is persisted in a file.
'''
...
class PropertyValueCache:
    ...
    It allows lookup for an entry by any of the values for a set of key properties.
    The entries are loaded from a file and persisted in the file after updates.
    '''
        ...
        for file in file_util.file_get_matching(filename):
            with file_util.FileIO(filename) as csv_file:
                ...
                for row in csv_reader:
                    ...
                    self.add(row)
            logging.info(
                f'Loaded {num_rows} with columns: {self._props} from {filename} into cache'
            )
    ...
    def save_cache_file(self):
        ...
        logging.info(
            f'Writing {len(self._entries)} cache entries with columns {self._props} into file {filename}'
        )
        with file_util.FileIO(filename, mode='w') as cache_file:
            csv_writer = csv.DictWriter(cache_file,
