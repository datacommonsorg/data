...
class TestStatVarProcessor(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.test_files = [
        ...
        os.path.join(_SCRIPT_DIR, 'test_data', 'us_census_EC1200A1-2022-09-15'),
  ...
  def compare_mcf_files(self, file_pairs: dict):
    ...
    for actual_file, expected_file in file_pairs.items():
      counters = Counters()
      diff = diff_mcf_files(
          actual_file, expected_file, {'show_diff_nodes_only': True}, counters
      )
      self.assertEqual(
          diff,
          '',
  ...
  def compare_csv_files(self, file_pairs: dict):
    ...
    for actual_file, expected_file in file_pairs.items():
      ...
      df_expected.sort_values(
          by=df_expected.columns.to_list(), inplace=True, ignore_index=True
      )
      df_actual.sort_values(
          by=df_expected.columns.to_list(), inplace=True, ignore_index=True
      )
      self.assertTrue(
  ...
  def process_file(self, file_prefix: str):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
      ...
      test_pv_maps = [os.path.join(_SCRIPT_DIR, f'{file_prefix}_pv_map.py')]
      ...
      expected_test_output = os.path.join(_SCRIPT_DIR, f'{file_prefix}_output')
      output_files = {}
      for output_file in ['.csv', '.tmcf', '_stat_vars.mcf']:
        output_files[test_output + output_file] = (
            expected_test_output + output_file
        )
