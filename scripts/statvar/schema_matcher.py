...
flags.DEFINE_string(
    'output_csv', '', 'Output CSV with matching property, values.'
)
flags.DEFINE_string(
    'schema_mcf',
    '/usr/local/google/dc/stat_vars/all_schema.mcf',
    'Comma separated list of MCF files.',
)
flags.DEFINE_list(
    'mcf_include_pv', [], 'List of property:values to include in the MCF.'
)
...
class SchemaMatcher:
  ...
  def __init__(
      self, mcf_file: str, config: dict = {}, counters: Counters = None
  ):
    ...
    self.add_nodes_to_matcher(
        self.schema_nodes, self._config.get('match_props')
    )
  ...
  def add_nodes_to_matcher(
    ...
    for dcid, node in nodes.items():
      ...
      for prop in props:
        ...
        if prop_value:
          self._node_matcher.add_key_value(strip_namespace(prop_value), node)
  ...
  def lookup_pvs_for_query(self, query: str, types: list = []) -> dict:
    ...
    pvs = {}
    nodes = self._node_matcher.lookup(
    ...
    for dcid, node in self.schema_nodes.items():
      if self._node_is_type(dcid, 'Property'):
        ...
        for r in range_includes:
          self._node_parent_prop[strip_namespace(r)] = strip_namespace(dcid)
  ...
  def _node_get_property(self, dcid: str) -> str:
    ...
    """
    node_types = list(_get_set(self._node_get_property_value(dcid, 'typeOf')))
    node_types.extend(
  ...
  def _node_get_types(self, dcid: str) -> set:
    ...
    while new_parents:
      ...
      for parent in new_parents:
        ...
        node_parents = _get_set(self._node_get_property_value(parent, 'typeOf'))
        node_parents.update(
            _get_set(self._node_get_property_value(parent, 'subClassOf'))
        )
