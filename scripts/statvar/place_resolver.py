...
flags.DEFINE_string(
    'place_resolver_cache', '', 'Cache file to save resolved places.'
)
flags.DEFINE_string(
    'maps_api_cache', '', 'Cache file to save responses from maps API.'
)
flags.DEFINE_integer(
    'place_pprof_port', 0, 'HTTP port for running pprof server.'
)
flags.DEFINE_string(
    'resolve_api_url',
...
class PlaceResolver:
  ...
  def resolve_name(
      ...
      if 'dcid' not in result:
        ...
        for place_id in place_ids:
          ...
          if cached_place:
            ...
            self._counters.add_counter('dc-api-resolve-placeid-cache-hits', 1)
          else:
    ...
    if lookup_placeids:
      ...
      self._counters.add_counter(
          'dc-api-resolve-placeid-lookups', len(lookup_placeids)
      )
        ...
        if 'dcid' not in result:
          ...
          for index in range(len(place_ids)):
            ...
            if dcid:
              ...
              self._set_cache_value('', {'placeId': place_id, 'dcid': dcid})
    ...
    if unresolved_places and self._place_name_matcher:
      ...
      self._counters.add_counter(
          'dc-api-unresolved-places', len(unresolved_places)
      )
      name_results = self.lookup_names(unresolved_places)
      results.update(name_results)
    logging.debug(f'Resolved names: {results}')

    filtered_results = self.filter_by_pvs(
        results, place_types, places_within, filter_pvs
    )
  ...
  def resolve_name_dc_api_batch(self, place_names: list) -> dict:
    ...
    while index < num_places:
      ...
      self._counters.add_counter(
          'dc-api-resolve-name-lookups', len(params['nodes'])
      )
      self._counters.add_counter('dc-api-resolve-name-calls', 1)
      batch_resp = request_url(
          url, method='POST', headers=headers, params=params, output='json'
      )
      ...
      if not batch_resp:
        ...
        for resp in batch_resp.get('entities', []):
          if 'resolvedIds' in resp:
            ...
            self._counters.add_counter('dc-api-resolve-name-dcids', 1)
  ...
  def resolve_latlng(self, places: dict) -> dict:
    ...
    for key, place in places.items():
      ...
      if lat and lng:
        ...
        if cached_place:
          self._counters.add_counter(f'dc-api-resolve-latlng-cache-hits', 1)
    ...
    if resolve_places:
      ...
      self._counters.add_counter(
          f'dc-api-resolve-latlng-places', len(resolve_places)
      )
  ...
  def get_maps_placeid(
    ...
    if self._config.get('maps_api_text_search', True):
      return self.lookup_maps_placeid(name, country, admin_area, place_types)
    ...
    if not self._maps_api_key:
      logging.error(f'No maps key. Please set --maps_api_key for place lookup.')
    params = {
    ...
    if resp_json:
      ...
      if 'results' in resp_json:
        ...
        if len(results) > 0:
          ...
          if result:
            self._counters.add_counter('maps-api-geocode-results', 1)
      # Cache the response
  ...
  def lookup_maps_placeid(
    ...
    resp_json = request_url(
        url=_MAPS_TEXT_SEARCH_URL, params=params, output='json'
    )
    result = {}
    if resp_json:
        ...
        for map_result in map_results:
          ...
          if 'geometry' in map_result:
            if 'location' in map_result['geometry']:
              ...
              _add_to_dict('longitude', loc.get('lng', ''), result)
      if result:
        self._set_cache_value(place_key, result)
  ...
  def filter_by_pvs(
    ...
    places_within.update(_get_value_set(self._config.get('places_within', [])))
    if places_within:
      places_within.update(filter_value_set.get('containedInPlace', set()))
    ...
    for prop, dcids in lookup_dcids_by_prop.items():
      ...
      self._counters.add_counter(
          f'dc-api-prop-value-{prop}-lookups', len(dcids)
      )
      ...
      for prop, filter_values in filter_value_set.items():
        ...
        if value:
          ...
          if not place_values.intersection(filter_values):
            ...
            break
          self._counters.add_counter(f'place-prop-filter-{prop}-allowed', 1)
      if allow_place:
        filtered_places[place_key] = place_props
    logging.debug(
        f'Returning {len(filtered_places)} filtered places:'
  ...
  def _get_unresolved_places(
      self, places: dict, unresolved_places: dict, resolved_places: dict
  ):
  ...
  def _load_cache(self):
    ...
    self._cache = PropertyValueCache(
        key_props=['place_name', 'dcid', 'placeId'],
  ...
  def _save_cache(self, time_interval: int = 0):
    ...
    if self._cache.is_dirty() and (
        self._cache_save_timestamp + time_interval <= time.perf_counter()
    ):
  ...
  def _get_cache_value(self, cache_key: str, prop: str = '') -> dict:
    ...
    if prop and cached_entry and cached_entry.get(prop):
      self._counters.add_counter(f'cache-hit-{prop}', 1)
      return dict(cached_entry)
    # See if place was looked up earlier and failed.
    failed_cache_entry = self._failure_cache.get_entry(prop='', value=cache_key)
  ...
  def _cache_contained_in_places(self, dcids: list, level: int = 0):
    ...
    for dcid in dcids:
      if dcid != 'Earth':
        parents = self._get_cache_value(dcid, 'containedInPlace').get(
            'containedInPlace'
        )
        if not parents:
    ...
    if lookup_dcids:
      ...
      self._counters.add_counter(
          f'dc-api-prop-value-containedInPlace-lookups', len(lookup_dcids)
      )
      # Cache the property:value for the response.
      for dcid, prop_values in dc_api_resp.items():
        if prop_values:
  ...
  def _get_contained_in_place(self, dcid: str) -> set:
    ...
    cached_parents = self._get_cache_value(dcid, 'containedInPlace').get(
        'containedInPlace'
    )
    parents.update(_get_value_set(cached_parents))
    new_parents = parents
    logging.debug(f'Got containedInPlace for {dcid} from cache: {new_parents}')
    ...
    while new_parents:
      ...
      for parent in lookup_parents:
        cached_parents = _get_value_set(
            self._get_cache_value(parent, 'containedInPlace').get(
                'containedInPlace'
            )
        )
...
def _get_maps_place_types(place_types: list) -> list:
  ...
  for place_type in place_types:
    if place_type.startswith('AdministrativeArea'):
      maps_types.append(
          place_type.replace('AdministrativeArea', 'administrative_area_level_')
      )
  return maps_types
...
def process(
  ...
  logging.info(
      f'Writing {len(resolved_places)} rows with columns: {output_columns} into'
      f' {output_filename}'
  )
