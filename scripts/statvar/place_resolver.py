...
flags.DEFINE_string('resolve_api_url',
...
_MAPS_TEXT_SEARCH_URL = 'https://maps.googleapis.com/maps/api/place/textsearch/json'
  ...
  '''
    ...
    def resolve_name(self,
        ...
        '''Returns a dictionary with dcid and placeId for each place.
         ...
         Args:
           ...
           filter_pvs:
             dictionary of property:values to filter results by.
    ...
    def lookup_names(self,
        ...
        for key, place in unresolved_places.items():
            ...
            logging.debug(
                f'Got name lookup results: {place_name}, {property_filters}: {lookup_results}'
            )
    ...
    def lookup_maps_placeid(self,
                            name: str,
                            country: str = None,
                            admin_area: str = '',
                            place_types: list = []) -> list:
    ...
    def filter_by_pvs(self,
        ...
        Args:
          places: dictionary that maps place_name to a dict of place property:values.
          filter_pvs: dictionary of allowed values for place properties.
            Only places with property values in the filter_pvs are returned.
        '''
        ...
        logging.debug(
            f'Filtering {places} with filter_pvs: {filter_value_set} with config {self._config.get_configs()}'
        )
            ...
            for prop, filter_values in filter_value_set.items():
                ...
                if value:
                    ...
                    if not place_values.intersection(filter_values):
                        ...
                        logging.debug(
                            f'Place {place_props} did not match {prop}:{place_values} in {filter_value_set}'
                        )
                        break
                    self._counters.add_counter(
        ...
        logging.debug(
            f'Returning {len(filtered_places)} filtered places: {filtered_places} that match {filter_value_set}'
        )
        return filtered_places

    def _get_lookup_name(self, key: str, values: dict) -> str:
        '''Returns the name to be looked up for the request entry.
        If values has the key, 'place_name' and 'country' it is used.
    ...
    def _get_contained_in_place(self, dcid: str) -> set:
        ...
        while new_parents:
            ...
            logging.debug(
                f'Got containedInPlace for {dcid}: {new_parents} from {lookup_parents}'
            )
...
def process(input_filenames: list,
    ...
    for filename in file_util.file_get_matching(input_filenames):
        with file_util.FileIO(filename) as csvfile:
            ...
            logging.info(
                f'Loading places from csv file {filename} with columns: {reader.fieldnames}...'
            )
            for row in reader:
                num_inputs += 1
    ...
    logging.info(
        f'Writing {len(resolved_places)} rows with columns: {output_columns} into {output_filename}'
    )
...
def main(_):
    ...
    process(_FLAGS.resolve_input_csv, _FLAGS.resolve_output_csv,
            _FLAGS.resolve_place_names, _FLAGS.maps_key, config)
