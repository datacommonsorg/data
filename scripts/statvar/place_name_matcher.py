...
flags.DEFINE_string(
    'input_csv', '', 'CSV file with names of places to resolve.'
)
flags.DEFINE_string(
    'input_name_column', 'name', 'CSV column for names in the input_csv.'
)
flags.DEFINE_list(
    'place_within',
    '',
...
flags.DEFINE_string(
    'ngram_matcher_config', '', 'Configuration settings for ngram matcher.'
)
flags.DEFINE_list(
    'lookup_names', '', 'Comma separated list of places to lookup.'
)
flags.DEFINE_string('output_csv', '', 'Output CSV with place dcids added.')
flags.DEFINE_string(
    'place_csv', '', 'CSV file with place names and dcids to match.'
)
...
class PlaceNameMatcher:
  ...
  def __init__(
      self, place_file: str = '', places_within: list = [], config: dict = {}
  ):
  ...
  def _load_places_dict(self, place_csv: str, places_within: list):
    ...
    for file in file_util.file_get_matching(place_csv):
      ...
      if file_size > self._config.get('max_places_csv_file_size', 10000000):
  ...
  def get_place_value(
      self, place_dcid: str, prop: str, default: str = ''
  ) -> str:
  ...
  def lookup(
    ...
    matches = self._ngram_matcher.lookup(
        key=place_name, num_results=num_results, config=self._config
    )
    logging.debug(
  ...
  def process_csv(self, input_csv: str, name_column: str, output_csv: str):
    ...
    with file_util.FileIO(output_csv, mode='w') as csv_output:
      ...
      for file in files:
        counters.add_counter('total', file_util.file_estimate_num_rows(file))
      for file in files:
          ...
          for row in csv_reader:
            ...
            if place_name:
              ...
              if matches:
                ...
                logging.debug(f'Found matches for {row}: {matches}')
...
def main(_):
  ...
  place_name_matcher = PlaceNameMatcher(
      _FLAGS.place_csv, _FLAGS.place_within, config
  )
  if _FLAGS.input_csv:
