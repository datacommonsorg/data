...
class PropertyValueMapper:
  ...
  '''
    ...
    def get_all_pvs_for_value(self,
      ...
      '''
        logging.level_debug() and logging.log(
            3, f'Looking up PVs for {namespace}:{value}')
        if not value:
            return None
...
class StatVarsMap:
    ...
    def write_statvar_obs_csv(self,
        ...
        logging.info(
            f'Writing {len(self._statvar_obs_map)} SVObs  into {output_csv} with {columns}'
        )
        svobs_unique_values = {}
...
class StatVarDataProcessor:
    ...
    def process_row(self, row: list, row_index: int):
        ...
        for col_index in range(len(row)):
            ...
            if col_pvs:
                ...
                col_numeric_val = get_numeric_value(
                    col_value, self._config.get('number_decimal', '.'),
        ...
        if not row_svobs and cols_with_pvs > 0 and self.is_header_index(
        ...
        for statvar_prop in statvar_ref_props:
            for pvs in [statvar_pvs, svobs_pvs]:
                if pvs and statvar_prop in pvs:
                    ...
                    logging.level_debug() and logging.debug(
                        f'Generating statvar for {statvar_prop}:{prop_value} with {new_statvar_pvs} from {pvs}'
                    )
    ...
    def write_outputs(self, output_path: str):
        ...
        if self._config.get('generate_statvar_mcf', True):
            ...
            statvar_mcf_file = self._config.get('output_mcf',
                                                output_path + '.mcf')
        ...
        if self._config.get('generate_csv', True):
            ...
            output_csv = self._config.get('output_csv_file',
                                          output_path + '.csv')
            output_tmcf_file = ''
