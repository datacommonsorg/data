...
class StatVarsMap(Config, Counters):
    ...
    def write_statvars_mcf(self,
        ...
        logging.info(
            f'Generating {len(self._statvars_map)} statvars into {filename}')
        write_mcf_nodes([self._statvars_map], filename, mode)
...
class StatVarDataProcessor(Config, Counters):
    ...
    def process_stat_var_obs(self, pvs: dict) -> bool:
        ...
        pvs = self.preprocess_stat_var_pbs_pvs(pvs)
        if not pvs:
            _DEBUG and logging.debug(
        ...
        if not self.resolve_svobs(svobs_pvs):
            logging.error(f'Unable to resolve SVObs place {pvs}')
            return False
    ...
    def resolve_svobs(self, pvs: dict) -> bool:
        ...
        logging.error(f'Unable to resolve place {place} in {pvs}')
        self.add_counter(f'error-resolve-place', 1,
                         pvs.get('variableMeasured', ''))
        return False
    ...
    def write_outputs(self, output_path: str):
        ...
        if self.get_config('generate_statvar_mcf', True):
            self._statvars_map.write_statvars_mcf(output_path + '.mcf')
...
def shard_csv_data(files: list,
    ...
    dfs = []
    for file in files:
        df.append(pd.read_csv(file, dtype=str))
    df = pd.concat(dfs)
    if not column:
