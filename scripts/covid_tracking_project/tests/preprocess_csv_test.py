...
class TestCovidIntegration(unittest.TestCase):

    def test_cleaned_csv_and_tmcf(self):
        output_columns = [
        ...
        with open('test_cleaned_output.csv', 'w', newline='') as f_out:
            writer = csv.DictWriter(f_out,
                                    fieldnames=output_columns,
                                    lineterminator='\n')
            with open('test_input.csv') as response:
...
Node: E:test_cleaned_output->E{index}
typeOf: dcs:StatVarObservation
variableMeasured: dcs:{stat_var}
measurementMethod: dcs:CovidTrackingProject
observationAbout: C:test_cleaned_output->GeoId
observationDate: C:test_cleaned_output->Date
value: C:test_cleaned_output->{stat_var}
"""

        stat_vars = output_columns[2:]
        with open('test_tmcf_output.tmcf', 'w', newline='') as f_out:
