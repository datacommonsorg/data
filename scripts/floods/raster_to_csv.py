...
python3 raster_to_csv.py \
      ...
      --aggregate_s2_level=10 `# Generate S2 cells for all levels upto 10` \
      --contained_in_s2_level=10  `# Generate places for all s2 cells upto level 10` \
...
2. Place files for new S2 cells Ids generated by this data set.
   2.1 Place csv file <place-csv-tmcf-prefix>.csv for s2 cells with
      the following columns:
          s2CellId,typeOf,containedInPlace
      where typeOf is of the form 'dcid:S2CellLevel<N>'
        containeInPlaces is alist of s2 cell ids for all parent levels upto
          --contained_in_s2_level
...
flags.DEFINE_integer('contained_in_s2_level', 10,
                     'Top contained in S2 level in --output_s2_place csv.')
...
_DEFAULT_CONFIG = {
    ...
    'contained_in_s2_level': _FLAGS.contained_in_s2_level,
...
def write_s2place_csv_tmcf(data_points: dict,
    ...
    Args:
      ...
      config: dictionary of config parameters
        contained_in_s2_level: topmost s2 level upto which place are added.
        aggregate_s2_level: topmost s2 cell level upto which data points are generated
      counter: dictionary of named counters
    '''
    s2_places = {}
    top_s2_level = config.get('aggregate_s2_level', None)
    top_s2_level = config.get('contained_in_s2_level', top_s2_level)
    ...
    for data in data_points.values():
        ...
        if not s2cell:
            continue
        s2_places[s2cell.id()] = {}
        for level in range(top_s2_level, s2cell.level()):
            s2_places[s2cell.parent(level).id()] = {}
    logging.info(
        f'Generating place data for {len(s2_places)} places upto level {top_s2_level} into {output_prefix}.csv/tmcf'
    )
