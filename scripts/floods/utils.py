...
def grid_id_from_lat_lng(degrees: float,
    ...
    lat_str = str_from_number(int(lat / degrees) * degrees)
    lng_str = str_from_number(int(lng / degrees) * degrees)
...
def grid_id_to_deg_lat_lng(
    ...
    if deg is not None:
        ...
        deg = default_deg
    if deg > 10:
        # Decimal degrees are prepresented with fractional part.
        deg = deg / 100
    if lat_lng:
    ...
    for lat_offset in [-1, 0, 1]:
        for lng_offset in [-1, 0, 1]:
            if lat_offset != 0 or lng_offset != 0:
                ...
                if abs(neighbour_lat) < 180.0 and abs(neighbour_lng) < 90:
                    neighbours.append(
    ...
    for filename in input_files:
        ...
        if file_size:
            if filename.endswith('.pkl'):
                ...
                logging.info(f'Loaded py dict of size: {file_size} from {filename}')
    return py_dict
