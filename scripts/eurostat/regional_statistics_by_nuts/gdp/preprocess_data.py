...
def first_stage_processing(df, separation_symbol):
    ...
    combined_variable_name = combined_variable_name[:-len(
        separation_symbol)]  # remove last underscore
    ...
    for i in range(len(df)):
        ...
        for j in range(len(time_columns)):
            ...
            all_records.append([
                time_invariant_var_value, time_columns[j],
                current_combined_variable_value, value, note
            ])
    new_df = pd.DataFrame(all_records,
                          columns=[
                              time_invariant_var, 'time',
                              combined_variable_name, 'value', 'note'
                          ])
...
def second_stage_processing(df, separation_symbol):
    ...
    combined_variable_values_list = sorted(
        list(df[combined_variable_name].unique()))
    ...
    if 'unit' in combined_variable_name:
        unit_index = combined_variable_name.split(separation_symbol).index(
            'unit')
        for v in combined_variable_values_list:
    ...
    for name, group in grouped_df:
        ...
        missing_variable_list = list(combined_variables_values_set -
                                     set(variable_list))
        ...
        variable_list_sorted = [
            variable for variable, value, note in variable_value_note_sorted
        ]
        assert variable_list_sorted == combined_variable_values_list
        value_list_sorted = [
            value for variable, value, note in variable_value_note_sorted
        ]
        note_list_sorted = [
            note for variable, value, note in variable_value_note_sorted
        ]
        if len(units_list) > 0:
    ...
    if len(units_list) > 0:
        new_df = pd.DataFrame(all_records,
                              columns=['geo', 'time'] +
                              combined_variable_values_list + notes_list +
                              units_list)
    else:
        new_df = pd.DataFrame(all_records,
                              columns=['geo', 'time'] +
...
def main(args):
    ...
    if args.output_file is None:
        second_stage_df.to_csv(args.input_file.replace('.tsv.gz',
                                                       '_processed.csv'),
                               index=False)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i',
                        dest='input_file',
                        default='demo_r_pjangrp3.tsv.gz')
    parser.add_argument('-o', dest='output_file', default=None)
    parser.add_argument('-s', dest='sep', type=str,
