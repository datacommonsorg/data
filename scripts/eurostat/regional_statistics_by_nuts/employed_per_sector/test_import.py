...
import unittest
import os
from emp_persec_preprocess_gen_tmcf import *
import pandas as pd
import tempfile
import sys
...
class TestProcess(unittest.TestCase):
    ...
    def __init__(self, methodName: str = ...) -> None:
        ...
        input_df = pd.read_table(
            os.path.join(TEST_DATASET_DIR, "sample_data.tsv"))
        input_df = input_df.rename(
            columns={
                'freq,unit,wstatus,nace_r2,geo\TIME_PERIOD':
                    'unit,wstatus,nace_r2,geo\\time'
            })
        input_df['unit,wstatus,nace_r2,geo\\time'] = input_df[
    ...
    def test_country_data(self):
        ...
        with open(self.CLEANED_CSV_FILE_PATH,
                  encoding="utf-8") as cleaned_csv_file:
            for line in cleaned_csv_file:
                if "2001,country/AUT,230600.0,709900.0,655900.0,267400.0,1042700.0,1140000.0,97300.0,126800.0,485800.0,54500.0,304500.0,797900.0,948300.0,150300.0,3782000.0" in line:
                    self.assertEqual(True, True)

    def test_nuts_data(self):
        """
        This method is required to test between output generated
        preprocess script and excepted output files like CSV
        """
        ...
        with open(self.CLEANED_CSV_FILE_PATH,
                  encoding="utf-8") as cleaned_csv_file:
            for line in cleaned_csv_file:
                if "2002,nuts/AT1,68600.0,227700.0,205700.0,102900.0,439400.0,503900.0,64500.0,61000.0,246600.0,32000.0,153600.0,363600.0,440700.0,77100.0,1590400.0" in line:
                    self.assertEqual(True, True)
