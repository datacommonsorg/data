...
from common.replacement_functions import (
    _replace_sex, _replace_isced11, _replace_isced97, _replace_n_portion,
    _replace_deg_urb, _replace_quant_inc, _replace_bmi, _replace_c_birth,
    _replace_citizen, _replace_coicop, _replace_frequenc, _replace_lev_limit,
    _split_column)
...
def _consumption_fruit_vegetables_edu(df: pd.DataFrame) -> pd.DataFrame:
  ...
  """
    ...
    'ConsumptionOfFruitsOr'+'ConsumptionOfVegetables_'+'In_Count_Person_'+\
    df['isced11']+'_' + df['sex']
    df.drop(columns=['unit', 'age', 'isced11', 'n_portion', 'sex'],
            inplace=True)
...
def _consumption_fruit_vegetables_inc(df: pd.DataFrame) -> pd.DataFrame:
  ...
  """
    ...
    df['SV']= 'Percent_'+'Daily_'+df['n_portion']+'_'+'ConsumptionOfFruitsOr'+\
              ...
              '_'+ df['quant_inc']
    df.drop(columns=['unit', 'age', 'quant_inc', 'n_portion', 'sex'],
            inplace=True)
    df = df.melt(id_vars=['SV', 'geo'],
                 var_name='time',
                 value_name='observation')
    return df
...
def _consumption_fruit_vegetables_urb(df: pd.DataFrame) -> pd.DataFrame:
  ...
  """
    ...
    df['SV'] = 'Percent_'  +\
    'Daily_'+df['n_portion']+'_'+'ConsumptionOfFruitsOr'+\
    'ConsumptionOfVegetables_'+'In_Count_Person_'+df['deg_urb']+'_'+df['sex']
    df.drop(columns=['unit', 'age', 'deg_urb', 'n_portion', 'sex'],
            inplace=True)
...
Args:
 ...
 """
    cols = [
        'unit,n_portion,sex,age,c_birth,time', 'EU27_2020', 'EU28', 'BE', 'BG',
        'CZ', 'DK', 'DE', 'EE', 'IE', 'EL', 'ES', 'FR', 'HR', 'IT', 'CY', 'LV',
        'LT', 'LU', 'HU', 'MT', 'NL', 'AT', 'PL', 'PT', 'RO', 'SI', 'SK', 'FI',
        'SE', 'IS', 'NO', 'UK', 'TR'
    ]
    ...
    'ConsumptionOfFruitsOr'+'ConsumptionOfVegetables_'+'In_Count_Person_'+\
    df['sex']+'_'+df['c_birth']
    df.drop(columns=['unit', 'age', 'n_portion', 'c_birth', 'sex'],
            inplace=True)
    df = df.melt(id_vars=['SV','time'], var_name='geo'\
            ,value_name='observation')
    return df
...
def _consumption_fruit_vegetables_coc(df: pd.DataFrame) -> pd.DataFrame:
 ...
 """
    cols = [
        'unit,n_portion,sex,age,citizen,time', 'EU27_2020', 'EU28', 'BE', 'BG',
        'CZ', 'DK', 'DE', 'EE', 'IE', 'EL', 'ES', 'FR', 'HR', 'IT', 'CY', 'LV',
        'LT', 'LU', 'HU', 'MT', 'NL', 'AT', 'PL', 'PT', 'RO', 'SI', 'SK', 'FI',
        'SE', 'IS', 'NO', 'UK', 'TR'
    ]
    ...
    df.drop(columns=['unit', 'age', 'n_portion', 'citizen', 'sex'],
            inplace=True)
    df = df.melt(id_vars=['SV','time'], var_name='geo'\
            ,value_name='observation')
    return df
...
def _consumption_fruit_vegetables_lev(df: pd.DataFrame) -> pd.DataFrame:
 ...
 """
    cols = [
        'n_portion,lev_limit,sex,age,unit,time', 'EU27_2020', 'EU28', 'BE',
        'BG', 'CZ', 'DK', 'DE', 'EE', 'IE', 'EL', 'ES', 'FR', 'HR', 'IT', 'CY',
        'LV', 'LT', 'LU', 'HU', 'MT', 'NL', 'AT', 'PL', 'PT', 'RO', 'SI', 'SK',
        'FI', 'SE', 'IS', 'NO', 'UK', 'TR'
    ]
    ...
    df.drop(columns=['unit', 'age', 'n_portion', 'lev_limit', 'sex'],
            inplace=True)
    df = df.melt(id_vars=['SV','time'], var_name='geo'\
            ,value_name='observation')
    return df
...
def _consumption_fruit_vegetables_bmi(df: pd.DataFrame) -> pd.DataFrame:
 ...
 """
    cols = [
        'unit,n_portion,sex,age,bmi,time', 'EU27_2020', 'BE', 'BG', 'CZ', 'DK',
...
def _frequency_fruit_vegetables_cob(df: pd.DataFrame) -> pd.DataFrame:
 ...
 """
    cols = [
        'unit,frequenc,coicop,c_birth,sex,age,time', 'EU27_2020', 'EU28', 'BE',
        'BG', 'CZ', 'DK', 'DE', 'EE', 'IE', 'EL', 'ES', 'FR', 'HR', 'IT', 'CY',
        'LV', 'LT', 'LU', 'HU', 'MT', 'NL', 'AT', 'PL', 'PT', 'RO', 'SI', 'SK',
        'FI', 'SE', 'IS', 'NO', 'UK', 'TR'
    ]
...
def _frequency_fruit_vegetables_coc(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    cols = [
        'unit,frequenc,coicop,sex,age,citizen,time', 'EU27_2020', 'EU28', 'BE',
        'BG', 'CZ', 'DK', 'DE', 'EE', 'IE', 'EL', 'ES', 'FR', 'HR', 'IT', 'CY',
        'LV', 'LT', 'LU', 'HU', 'MT', 'NL', 'AT', 'PL', 'PT', 'RO', 'SI', 'SK',
        'FI', 'SE', 'IS', 'NO', 'UK', 'TR'
    ]
...
def _frequency_fruit_vegetables_inc(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    cols = [
        'unit,frequenc,coicop,sex,age,quant_inc,time', 'EU27_2020', 'BE', 'BG',
        'CZ', 'DK', 'DE', 'EE', 'IE', 'EL', 'ES', 'FR', 'HR', 'IT', 'CY', 'LV',
        'LT', 'LU', 'HU', 'MT', 'NL', 'AT', 'PL', 'PT', 'RO', 'SI', 'SK', 'FI',
        'SE', 'IS', 'NO', 'RS', 'TR'
    ]
...
def _frequency_fruit_vegetables_bmi(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    cols = [
        'unit,frequenc,coicop,sex,age,bmi,time', 'EU27_2020', 'BE', 'BG', 'CZ',
    ...
    df.drop(columns=['unit', 'age', 'bmi', 'coicop', 'frequenc', 'sex'],
            inplace=True)
    df = df.melt(id_vars=['SV','time'], var_name='geo'\
            ,value_name='observation')
    return df
...
def _frequency_fruit_vegetables_lev(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    cols = [
        'frequenc,coicop,lev_limit,sex,age,unit,time', 'EU27_2020', 'EU28',
        'BE', 'BG', 'CZ', 'DK', 'DE', 'EE', 'IE', 'EL', 'ES', 'FR', 'HR', 'IT',
        'CY', 'LV', 'LT', 'LU', 'HU', 'MT', 'NL', 'AT', 'PL', 'PT', 'RO', 'SI',
        'SK', 'FI', 'SE', 'IS', 'NO', 'UK', 'TR'
    ]
    df.columns = cols
    df = _split_column(df, cols[0])
    # Filtering out the wanted rows and columns
...
def _drinking_sugar_sweetened_bmi(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    cols = [
        'unit,frequenc,sex,age,bmi,time', 'EU27_2020', 'BE', 'BG', 'CZ', 'DK',
        'DE', 'EE', 'IE', 'EL', 'ES', 'FR', 'HR', 'IT', 'CY', 'LV', 'LT', 'LU',
...
def _drinking_sugar_sweeetened_inc(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    cols = [
        'unit,frequenc,sex,age,quant_inc,time', 'EU27_2020', 'BE', 'BG', 'CZ',
        'DK', 'DE', 'EE', 'IE', 'EL', 'ES', 'FR', 'HR', 'IT', 'CY', 'LV', 'LT',
        'LU', 'HU', 'MT', 'AT', 'PL', 'PT', 'RO', 'SI', 'SK', 'FI', 'SE', 'IS',
        'NO', 'RS', 'TR'
    ]
...
def _frequency_drinking_pure_juice_edu(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    cols = [
        'unit,sex,age,frequenc,isced11,time', 'EU27_2020', 'BE', 'BG', 'CZ',
        'DK', 'DE', 'EE', 'IE', 'EL', 'ES', 'FR', 'HR', 'IT', 'CY', 'LV', 'LT',
        'LU', 'HU', 'MT', 'AT', 'PL', 'PT', 'RO', 'SI', 'SK', 'FI', 'SE', 'IS',
        'NO', 'RS', 'TR'
    ]
...
def _consumption_fruits_edu(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    cols = [
        'sex,age,frequenc,isced97,time', 'BE', 'BG', 'CZ', 'EE', 'EL', 'ES',
        'FR', 'CY', 'LV', 'HU', 'MT', 'PL', 'RO', 'SI', 'SK', 'TR'
    ]
    df.columns = cols
    df = _split_column(df, cols[0])
    # Filtering out the wanted rows and columns
...
def main(_):
    ...
    mcf_name = "eurostat_population_consumptionoffruitsandvegetables.mcf"
    ...
    loader = EuroStatConsumptionOfFruitsandVegetables(ip_files,
                                                      cleaned_csv_path,
                                                      mcf_path, tmcf_path)
    loader.process()
