...
from common.replacement_functions import (
   _replace_col_values,_split_column)
...
def _consumption_fruit_vegetables_edu(df: pd.DataFrame) -> pd.DataFrame:
 ...
 """
   ...
   df = _replace_col_values(df,'n_portion')
...
def _frequency_fruit_vegetables_edu(df: pd.DataFrame) -> pd.DataFrame:
 ...
 """
   ...
   df = df[df['age'] == 'TOTAL']
   df = df[(df['geo'] != 'EU27_2020') & (df['geo'] != 'EU28')]
   df = _replace_col_values(df,'coicop')
   df = _replace_col_values(df,'sex')
   df = _replace_col_values(df,'isced11')
   df = _replace_col_values(df,'frequenc')
...
def _consumption_fruits_edu(df: pd.DataFrame) -> pd.DataFrame:
   ...
   df = _replace_col_values(df,'frequenc')
   df = _replace_col_values(df,'sex')
   df = _replace_col_values(df,'isced97')
   df['SV'] = 'Percent_'+ df['frequenc']+ '_'+\
