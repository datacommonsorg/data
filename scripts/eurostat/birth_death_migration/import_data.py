...
def preprocess_data(raw_df):
    ...
    preprocessed_df.value = preprocessed_df.value.str.replace("([0-9:])$", lambda m: m.group(0) + ' ')
    ...
    preprocessed_df = (preprocessed_df.set_index(["geo", "time"])
    ...
    preprocessed_df[preprocessed_df.columns[2:]] = preprocessed_df[
        preprocessed_df.columns[2:]].replace("([0-9:])$", lambda m: m.group(0) + ' ')
    ...
    for column_name in preprocessed_df.columns[2:]:
        preprocessed_df['{0}'.format(column_name)], preprocessed_df['{0}|notes'.format(column_name)] = (
            zip(*preprocessed_df[column_name].apply(lambda x: x.split(' '))))
...
def clean_data(preprocessed_df):
    ...
    clean_df = preprocessed_df[preprocessed_df.columns[:len(
        preprocessed_df.columns) // 2 + 1]]  # number of columns should be 2 + 2X, we want the first 2 + X
    # columns
    ...
    original_names = ['geo', 'time', 'DEATH', 'GBIRTHRT', 'GDEATHRT', 'GROW', 'GROWRT', 'JAN', 'LBIRTH']
    new_names = ['geo', 'time', 'Count_MortalityEvent', 'Count_BirthEvent_AsAFractionOfCount_Person',
                 'Count_MortalityEvent_AsAFractionOfCount_Person', 'IncrementalCount_Person', 'GrowthRate_Count_Person',
                 'Count_Person', 'Count_BirthEvent']
    clean_df = clean_df[original_names]
    clean_df[['GBIRTHRT', 'GDEATHRT', 'GROWRT']] /= 1000  # apply scaling factor of 1000
