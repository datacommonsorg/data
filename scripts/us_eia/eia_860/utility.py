...
UTILITY_DATATYPE = {
    ...
    'State': str,
    'Zip': str,
    'Owner of Plants Reported on Form?': str,
    'Operator of Plants Reported on Form?': str,
    'Asset Manager of Plants Reported on Form?': str,
    'Other Relationships with Plants Reported on Form?': str,
    'Entity Type': str,
}
...
def _zip_to_dcid(zip: str) -> str:
    if zip.isnumeric():
        return f'dcid:zip/{zip.zfill(5)}'
    return ''

def _build_address(row: pd.Series) -> str:
    return f'\"{row["StreetAddress"]}, {row["City"]}, {row["State"]} {row["Zip"]}\"'
...
def _update_frames(raw_df: pd.DataFrame) -> pd.DataFrame:
    ...
    raw_df['Name'] = raw_df['Name'].apply(_escape_name)
    ...
    raw_df['OwnerEnum'] = raw_df['IsOwner'].apply(_to_assoc_enum, dcid='dcid:EIA_OwnerOfPowerPlants')
    raw_df['OperatorEnum'] = raw_df['IsOperator'].apply(_to_assoc_enum, dcid='dcid:EIA_OperatorOfPowerPlants')
    raw_df['AssetManagerEnum'] = raw_df['IsAssetManager'].apply(_to_assoc_enum, dcid='dcid:EIA_AssetManagerOfPowerPlants')
    raw_df['OtherRelationshipEnum'] = raw_df['IsOther'].apply(_to_assoc_enum, dcid='dcid:EIA_OtherRelationshipWithPowerPlants')
    print(raw_df)
    return raw_df

def process(in_path: str, out_path: str):
    """Read data from excel and create CSV required for DC import"""
    raw_df = pd.read_excel(in_path, header=1, dtype=UTILITY_DATATYPE, names=UTILITY_IMPORT_COLUMNS)
    df = _update_frames(raw_df)
    df.to_csv(out_path, columns=UTILITY_EXPORT_COLUMNS, quoting=csv.QUOTE_ALL, escapechar='\\', doublequote=False, index=False)
