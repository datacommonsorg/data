...
_DEFAULT_STAT_VAR_PV = {
    ...
    'populationType': 'dcs:CommercialBuilding',
...
_DEFAULT_SVG_IGNORE_PROPS = _DCID_IGNORE_PROPS + [
    'Node',
...
_MCF_FILE_HEADER = """
# Autogenerated file.
# For more details, please refer to https://github.com/datacommonsorg/data/tree/master/scripts/us_eia/cbecs
"""
...
def _print_counters(counters: dict, steps=None):
    row_key = 'input_rows_processed'
    if steps is None or counters.get(row_key, 0) % steps == 0:
        print('\nSTATS:')
        for k in sorted(counters):
            print(f"\t{k:>30} = {counters[k]}")
        if 'input_rows_processed' in counters:
            start_ts = counters['time_start']
            end_ts = time.perf_counter()
            print(
                'Processing rate: {:.2f}'.format(
                    counters['input_rows_processed'] / (end_ts - start_ts)),
                'rows/sec')
        print('', flush=True)


def _add_stat_var_prop_counts(sv_pvs: dict, sv_pv_counts: dict):
    '''Add all statvar PVs to the counters.'''
    for p in sv_pvs.keys():
        sv_pv_counts[p] = sv_pv_counts.get(p, 0) + 1
...
def get_stat_var_dcid(pvs: dict) -> dict:
    ...
    _DEFAULT_DCID_PROPS = [
        'statType', 'measurementQualifier', 'measuredProperty', 'populationType'
    ]
    ...
    for p in sorted(pvs.keys(), key=str.casefold):
        v = pvs[p]
...
def get_statvar_name(sv: dict) -> str:
    '''Returns the name string for a statvar.
...
def write_stat_var_mcf(sv_map: dict, filename: str, mode: str, config: dict,
                       default_props: dict, counters: dict):
    '''Write statvars to a file.'''
    _print_debug(0, f"Writing {len(sv_map)} stat-vars to {filename}.mcf")
    write_mcf_nodes([sv_map], filename + '.mcf', mode, header=_MCF_FILE_HEADER)
    _add_counter(f'output_nodes:{filename}', len(sv_map))
...
def write_statvar_obs_csv(sv_obs: list, filename: str, counters: dict):
    ...
    with open(filename + '.csv', 'w', newline='') as f_out_csv:
        ...
        for key, data_row in sv_obs.items():
            if data_row.get('value', '') == '':
                _add_error_counter(
                    'error_dropped_stat_var_obs_empty_value',
                    f'Dropping statvarobs with empty value: {data_row}',
                    counters)
            else:
...
def preprocess_row(row_text: list) -> list:
    ...
    row = [
        '' if x.startswith('Unnamed') else x.replace('- ', '') for x in row_text
    ]
    return row
...
def _get_svg_ignored_props(props: list) -> list:
    '''Return a set of properties that are ignored.'''
    ignored_props = []
    for p in props:
        # TODO(ajaits): Check if property is defined.
        if p in _DEFAULT_SVG_IGNORE_PROPS or not isinstance(
                p, str) or p[0].isupper():
            ignored_props.append(p)
    return ignored_props
...
def process_csv(csv_files: list, output_path: str, config: dict,
    ...
    for filename in csv_files:
        with open(filename, 'r', encoding='UTF8') as f:
            ...
            for row in csv_reader:
                ...
                _print_debug(3, f"Processing row {input_ref}:{row}")
                if num_rows >= 0 and counters['line'] > num_rows:
                    break
                if skip_rows > 0:
                    skip_rows -= 1
                    continue
                _add_counter('input_rows_processed', 1, counters)
                row = preprocess_row(row)
                if process_header_row(row, config, context, counters):
                    _print_debug(1, f"Processing header:{input_ref}: {row}")
                ...
                for column_number in range(1, len(row)):
                    ...
                    if _is_number(value):
                        ...
                        stat_var_pvs = get_stat_var_pvs(stat_var,
                                                        _OBSERVATION_PROPS)
                        _add_stat_var_prop_counts(stat_var_pvs,
                                                  stat_var_prop_counts)
    ...
    if svg_root != '' and svg_prefix != '':
        ignore_props = _get_svg_ignored_props(stat_var_prop_counts.keys())
        ...
        _print_debug(
            0,
            f'Generated {len(stat_var_groups)} svgs and {len(stat_var_members)} memberships for {len(stat_vars_map)} statvars'
        )
        _add_counter('stat_var_groups', len(stat_var_groups), counters)
        _add_counter('stat_var_group_members', len(stat_var_members), counters)
        write_stat_var_mcf(stat_var_groups, output_path + '.svg', 'w', config,
                           _DEFAULT_STAT_VAR_GROUP_PV, counters)
        write_stat_var_mcf(stat_var_members, output_path + '.svg', 'a', config,
                           _DEFAULT_STAT_VAR_GROUP_PV, counters)
    write_stat_var_mcf(stat_vars_map, output_path, 'w', config,
                       _DEFAULT_STAT_VAR_PV, counters)
