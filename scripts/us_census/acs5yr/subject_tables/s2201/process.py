...
def convert_column_to_stat_var(column):
  ...
  for p in s:
    ...
    # Only include the most specific enum value when there are multiple
    elif p == 'Other family:' and ('Male householder, no spouse present' in s or
                                   'Female householder, no spouse present'
                                   in s):
      continue
    elif p == 'Other family' and ('Male householder, no wife present' in s or
                                  'Female householder, no husband present'
                                  in s):
      continue
    elif p == 'One race' and (
      ...
      if ((p == 'No workers in past 12 months' or
           ...
           p == '2 or more workers in past 12 months') and 'Families' not in s):
  ...
  with open(output, 'w') as f_out:
    for i in range(len(stat_vars)):
      ...
      if 'Income' in stat_vars[i]:
        unit = _UNIT_TEMPLATE
      f_out.write(
          _TMCF_TEMPLATE.format(index=i, stat_var=stat_vars[i], unit=unit))
