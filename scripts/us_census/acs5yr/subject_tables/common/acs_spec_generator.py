    ...
    if 'populationType' in cur_spec:
      for population_token in cur_spec['populationType']:
        if not population_token.startswith('_'):
          ...
          elif out_spec['populationType'][population_token] != cur_spec[
              'populationType'][population_token]:
            new_token = population_token + ' XXXXX'
            temp_flag = True
            i = 2
            while new_token in out_spec['populationType']:
              if out_spec['populationType'][new_token] == cur_spec[
                  'populationType'][population_token]:
                temp_flag = False
              new_token += str(i)
              i += 1
        ...
        if not measurement_token.startswith('_'):
          if measurement_token not in out_spec['measurement']:
            ...
            while new_token in out_spec['measurement']:
              if out_spec['measurement'][new_token] == cur_spec['measurement'][
                  measurement_token]:
                temp_flag = False
              new_token += str(i)
              i += 1
            if temp_flag:
              out_spec['measurement'][new_token] = cur_spec['measurement'][
                  measurement_token]
        ...
        if not enum_token.startswith('_'):
          if enum_token not in out_spec['enumSpecializations']:
            ...
            print('Error:', enum_token,
                  ...
                  out_spec['enumSpecializations'][enum_token], 'new value:',
    ...
    for property_name in cur_spec['pvs']:
      ...
      for property_token in cur_spec['pvs'][property_name]:
        if property_token not in out_spec['pvs'][property_name]:
          out_spec['pvs'][property_name][property_token] = cur_spec['pvs'][
              property_name][property_token]
        elif out_spec['pvs'][property_name][property_token] != cur_spec['pvs'][
            property_name][property_token]:
          new_token = property_token + ' XXXXX'
          temp_flag = True
          i = 2
          while new_token in out_spec['pvs'][property_name]:
            if out_spec['pvs'][property_name][new_token] == cur_spec['pvs'][
                property_name][property_token]:
              temp_flag = False
            new_token += str(i)
            i += 1
          if temp_flag:
            out_spec['pvs'][property_name][new_token] = cur_spec['pvs'][
                property_name][property_token]
        ...
        if property_name not in out_spec['inferredSpec']:
          ...
          for dependent_prop in cur_spec['inferredSpec'][property_name]:
            if dependent_prop not in out_spec['inferredSpec'][property_name]:
              out_spec['inferredSpec'][property_name][
                  dependent_prop] = cur_spec['inferredSpec'][property_name][
                      dependent_prop]
              ...
              while new_token in out_spec['inferredSpec'][property_name]:
                if out_spec['inferredSpec'][property_name][
                    new_token] == cur_spec['inferredSpec'][property_name][
                        dependent_prop]:
                  temp_flag = False
                new_token += str(i)
                i += 1
              if temp_flag:
                out_spec['inferredSpec'][property_name][new_token] = cur_spec[
                    'inferredSpec'][property_name][dependent_prop]
...
def columns_from_zip_list(zip_path_list: list, check_metadata: bool = False) -> list:
...
def create_new_spec(all_columns: list,
  ...
  for population_token in union_spec['populationType']:
    if population_token.startswith('_'):
      ...
      if token_in_list_ignore_case(
          population_token[:population_token.find('XXXXX') - 1], all_tokens):
        out_spec['populationType'][population_token] = union_spec[
            'populationType'][population_token]
    else:
  ...
  for measurement_token in union_spec['measurement']:
    if measurement_token.startswith('_'):
      out_spec['measurement'][measurement_token] = union_spec['measurement'][
          measurement_token]
      ...
      if token_in_list_ignore_case(
          measurement_token[:measurement_token.find('XXXXX') - 1], all_tokens):
        out_spec['measurement'][measurement_token] = union_spec['measurement'][
            measurement_token]
    else:
  ...
  for prop in union_spec['pvs']:
    for property_token in union_spec['pvs'][prop]:
      if token_in_list_ignore_case(property_token, all_tokens):
        ...
        if token_in_list_ignore_case(
            property_token[:property_token.find('XXXXX') - 1], all_tokens):
          out_spec['pvs'][prop][property_token] = union_spec['pvs'][prop][
              property_token]
      else:
        if prop not in discarded_spec['pvs']:
          discarded_spec['pvs'][prop] = {}
        discarded_spec['pvs'][prop][property_token] = union_spec['pvs'][prop][
            property_token]
