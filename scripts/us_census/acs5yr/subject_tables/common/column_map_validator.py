...
def check_column_map(column_map_path: str,
  ...
  """
    ...
    with open(column_map_path, 'r') as fp:
        column_map = json.load(fp)

    column_list_path = os.path.expanduser(column_list_path)
    with open(column_list_path, 'r') as fp:
        column_year_list = json.load(fp)
    ...
    for year in column_year_list:
        temp_list = remove_columns_to_be_ignored(column_year_list[year],
                                                 spec_dict, delimiter)
    ...
    for year in dcid_list_all:
        if year != 'all':
            year_missing = list(
                set(dcid_list_all['all']) - set(dcid_list_all[year]))
...
if __name__ == '__main__':
    flags.mark_flags_as_required(
        ['spec_path', 'column_map', 'yearwise_columns'])
    app.run(main)
