...
sys.path.append(os.path.join(_SCRIPT_PATH, '../../../../../util/')) # for statvar_dcid_generator
...
def process_zip_file(zip_file_path, spec_path, write_output=True, output_dir_path='./', delimiter='!!', header_row=1):
          ...
          with io.TextIOWrapper(zf.open(filename, 'r')) as csv_file:
            ...
            for index, line in enumerate(csv_reader):
              if index == header_row:
                ...
                column_map[year] = generate_stat_var_map(spec_dict, line, delimiter)
                break
              continue
    ## save the column_map
    if write_output:
...
def generate_stat_var_map(spec_dict, column_list, delimiter='!!'):
  ...
  """
    col_map_obj = GenerateColMapBase(spec_dict=spec_dict, column_list=column_list, delimiter=delimiter)
...
class GenerateColMapBase:
  ...
  """
    ...
    def _column_to_statvar(self, column):
        ...
        if not measurement_assigned and '_DEFAULT' in self.features['measurement']:
        ...
        for elem in self.features['universePVs']:
            if stat_var['populationType'] == elem['populationType']:
                ...
                if (set(elem['constraintProperties']).issubset(set(list(stat_var.keys())))):
                  try:
                    ## if the dependentPVs are not in statVar add them
                    if not set(list(elem['dependentPVs'].keys())).issubset(set(list(stat_var.keys()))):
        ...
        #TODO: Use the _DISAMBIGUATING_POP_TYPE attribute in to clean stat_var dict
        ...
        stat_var_dcid = get_stat_var_dcid(stat_var, ignore_props=dependent_properties)
    ...
    def _keep_only_enum_specializations(self):
      ...
      And hence we do not add a stat var for "Under 19 Years". If the enumSpecialisation field is empty this will not be done.
      """
      ...
      for column_name, stat_var in column_map.items():
        ...
        if last_column_token  in self.features['enumSpecializations']:
          base_class = self.features['enumSpecializations'][last_column_token]
          for base in base_class.split(', '):
            if base in part_list:
              col_name = self.delimiter.join(part_list[:-1])
              try:
                del self.column_map[col_name]
                logger.info(f"Removing the base class column: {col_name} from the column map based on enumSpecialization of the spec")
              except KeyError:
                logger.info(f"Attempted removal of base class column: {col_name} failed. It might be already removed.")
