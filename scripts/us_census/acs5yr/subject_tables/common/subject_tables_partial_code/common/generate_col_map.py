...
Tasks:
-----
1. Add a dictionary of counters with summary stats of number of columns, number of statvars generated, statvars of differnt population types, errors or warnings
2. Should we generate an error _column_to_statvar() returns an empty statvar?
3. Before returning the column map, call self._isvalid_column_map() where we check of the same statvar is generated for more than one column? Should that be considered an error for subject tables?
"""
...
def process_csv_file(csv_file_path: str,
    ...
    if 'data_with_overlays' in filename:
        df = pd.read_csv(filename, header=header_row, low_memory=False, nrows=nrows)
