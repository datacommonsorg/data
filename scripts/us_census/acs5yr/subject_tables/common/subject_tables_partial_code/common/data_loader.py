...
Tasks
=====
1. Handle the open-distribution temp fix -- Ravi's data_validation.py adaptation
2. Chunk and parallelize large csv files
3. Avoid data holes if preceeding value is 0 -- measurementDenominator
4. Avoid data holes when computing percentages, if data point can be inferred as 0
5. Instead of writing to file in the data processing module, decouple the file write function. Outputs will dataframes, dictionaries. This makes the current implementation of the function to collect the outputs and return objects
6. Replace the next two lines with final_mcf = final_mcf + '\n'.join(col_stat_var) + '\n\n' modify the tmcf file content
7. Add 'Node:' as the first element of col_stat_var, and append all the stat-var node information to col_stat_var
8. If the data processing module is called from `process.py`, the message needs to be rephrased to suggest running `process.py` with column_map option
...
_GIT_HEAD_SHA = process.communicate()[0].strip().decode("utf-8")[:7]
...
def process_subject_tables(table_prefix: str = '',
                           input_path: str = './',
                           output_dir: str = './',
                           column_map_path: str = None,
                           spec_path: str = None,
                           debug: bool = False,
                           delimiter: str = '!!',
  ...
  has a human readable column header. (default: 1)
  years: it is a string that can be comma-separated to have multiple values that is used with has_percent flag. You will use this string to specify for which years do you want to convert percent to numbers
...
### TODO: chunk_size: for all csv files
class SubjectTableDataLoaderBase:
  ...
  """
    ...
    def __init__(self,
                 ...
                 json_spec: str = './',
                 column_map_path: str = './',
                 decimal_places: bool = 3,
                 estimate_period: bool = 5,
                 header_row=1,
                 years='all') -> None:
        """module init"""
        ## static inputs
        ...
        try:
            ...
            print(
                """To run this module in standalone mode, ensure the column is 
                generated first. Column map for a subject table dataset can be 
                generated through process.py with --option flag set as either 
                colmap or all. Alternatively, you can call the relevant method 
                in common/generate_col_map.py module based on the input file type"""
            )
            exit(1)
    ...
    def _convert_percent_to_numbers(self, df: pd.DataFrame,
                                    year: str) -> pd.DataFrame:
        """utility to convert percent to numbers"""
        if year not in self.years and 'all' not in self.years:
            print(year, "is skipped.. ")
            return df
        # TODO: Currently this function expects full names for function, which makes the spec long, perhaps we can add support for tokens
        df_columns = df.columns.tolist()
        for count_col, percent_col_list in self.json_spec['denominators'].items(
        ):
            if count_col in df_columns:
                for col in list(set(percent_col_list)):
                    if col in df_columns:
                        try:
                            df[col] = df[col].astype(float)
                        except ValueError:
                            # replaces +/- at the end of open distribution bounding values
                            df[col] = df[col].apply(
                                lambda x: re.sub("\W", "", x))
                            df[col] = df[col].astype(float)
                            print(
                                "Found symbols like + or - in the measurement, please run data_validation.py script"
                            )
    ...
    def _fill_holes_measurementDenominator(self, df):
        """
        Replaces cell value that are data suppressed with 0 if cell has value in _IGNROED_VALUES and the column specified as the key of the `denominators` section has the value = 0 for the row as the specified cell
        """
        return df
    ...
    def _process_dataframe(self, df, filename):
        ...
        if self.has_percent:
            df = self._convert_percent_to_numbers(df, year)
            df = self._fill_holes_measurementDenominator(df)
    ...
    def _generate_mcf_from_column_map(self):
        ...
        for column, stat_var in self.mcf_dict.items():
            ...
            for p, v in stat_var.items():
                if p != 'unit' or p != 'scalingFactor' or p != 'Node':
