...
def process_subject_tables(table_prefix='',
  ...
  corresponding processing function based on input format (*.zip/*.csv), output files are saved in the specified output directory.
  ...
  header_row: specifies the row index to be considered as the dataframe's header for column name assignment. For subject tables, the second row (row index = 1) has a human readable column header. (default: 1)
  ...
  """
    def __init__(self,
        ...
        try:
            ...
            # TODO: If the module is called from `process.py`, the message needs to be to run with --option=colmap. So rephrasing this message will help.
            print(
                "To run this module in standalone mode, ensure the column map is generated by running `generate_col_map` module."
            )
            exit(1)
    ...
    def _convert_percent_to_numbers(self, df, year):
              ...
              for col in list(set(percent_col_list)):
                    if col in df_columns:
                        try:
                          ...
                          df[col] = df[col].astype(float)
                          print("Found symbols like + or - in the measurement, please run data_validation.py script")
                        df[count_col] = df[count_col].astype(float)
                        df[col] = (df[col] / 100) * df[count_col]
                        df[col] = df[col].round(3) # TODO: replace with parameter
        return df

    def _get_summary(self):
        """method returns the total number of columns, stat vars and other stats from the colum_map and csv"""
    ...
    def _replace_ignore_values_with_nan(self, df):
        """Replaces cells of the dataframe by column with nan if cell has values in _IGNORED_VALUES"""
    ...
    def _process_dataframe(self, df, filename):
        ...
        try:
          ...
          print(f"Seems to have missing data for some geos. Please spot-check", flush=True)

    def _generate_mcf_from_column_map(self):
        """uses column_map to generate a mcf file with all the stat_vars nodes that are created"""
    ...
    def _get_outputs(self):
        ...
        f.close()
        print(
            f"processing of subject table is complete: the summary of the process is available at {summary_path} along with the generated output files"
        )
    ...
    def _process_dir(self, input_dir_path):
        ...
        try:
            ...
            print(
                "ensure input path is that of directory with dataset files, a zip file or a csv file"
            )
