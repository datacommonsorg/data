...
flags.DEFINE_string('table_prefix', None, '[for processing]Subject Table ID as a prefix for output files, eg: S2702')
...
flags.DEFINE_string('years', 'all', 'Set all if all years in the dataset needs percent to count conversion, else specify the specific years separated by commas like years=2010,2011 (default="all")')
...
def main(argv):
  ...
  # context: the current implementation of the column map generator accepts only zip files as input and we will need to add new methods to handle inputs as a directory of files or a single csv file.
  ...
  if file_extension == '.zip':
    ...
    if option == 'process':
      process_subject_tables(table_prefix=table_prefix, input_path=input_path, 
        output_dir=output_dir, column_map_path=os.path.join(output_dir, 'column_map.json'), spec_path=spec_path, debug=debug, delimiter='!!', has_percent=has_percent, years=FLAGS.years)
