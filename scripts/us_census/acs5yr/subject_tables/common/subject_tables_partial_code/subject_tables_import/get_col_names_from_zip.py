...
flags.DEFINE_string('zip_file_path', None, 'Path to the input zip file')
flags.DEFINE_string('table_id', None, 'Table ID')
...
def _get_column_names_across_years(zip_file_path, table_id, header=1,
                                   output_dir_path='./'):
  ...
  """
  df = pd.DataFrame(columns=['2010', '2011', '2012','2013','2014','2015',
                             '2016','2017', '2018', '2019'])
  zf = ZipFile(zip_file_path)
  for filename in zf.namelist():
    if 'data_with_overlays' in filename:
      data_df = pd.read_csv(zf.open(filename, 'r'), header=header, low_memory=False)
      year = filename.split(f'ACSST5Y')[1][:4]
      df[year] = data_df.columns.tolist()
  print(df.head())
  df.to_csv(f'{output_dir_path}/{table_id}_column_summary.csv', index=False)

def main(argv):
  """main method"""
  _get_column_names_across_years(
      FLAGS.zip_file_path,
      FLAGS.table_id,
      header=FLAGS.header,
