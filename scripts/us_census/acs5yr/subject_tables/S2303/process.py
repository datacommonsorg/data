...
class S2303SubjectTableDataLoader(SubjectTableDataLoaderBase):
    ...
    def _process_dataframe(self, df, filename):
        ...
        if 'data_with_overlays' in filename:
            ...
            df = df[~((df['Geography'].str.startswith('950')) |
                      (df['Geography'].str.startswith('960')) |
                      (df['Geography'].str.startswith('970')))]
        ...
        if 'data_with_overlays' in filename:
            ...
            place_geo_ids = df['Geography'].apply(convert_to_place_dcid)
...
def main(argv):
    ...
    data_loader = S2303SubjectTableDataLoader(table_id='S2303',
                                              col_delimiter='!!',
                                              has_percent=True,
                                              debug=True,
