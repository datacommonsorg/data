...
def _process_nationals_2029(file_path: str) -> pd.DataFrame:
    ...
    if dt.now().year > 2023 and str(dt.now().year) in data_df.columns:
        df_cols.append(dt.now().year)
    data_df.columns = df_cols
    data_df = data_df[data_df["Location"] == USA]
...
def _process_states_2029(file_path: str) -> pd.DataFrame:
    ...
    data_df = pd.DataFrame()
    data_df = _load_data_df(path=file_path, file_format="xlsx", header=8)
    df_cols = ["Region", "042020", "072020", "072021", "2022", "2023"]
    data_df.columns = df_cols
    if dt.now().year > 2023 and str(dt.now().year) in data_df.columns:
...
def _process_cities(file_path: str) -> pd.DataFrame:
    ...
    """
    data_df = None
    if 'sub-est2023.csv' in file_path:
        pass
    ...
    if file_name_without_ext in [
        ...
        place_data_df = data_df[(data_df['PLACE'] != 0) & (data_df['SUMLEV'] != 61)]
        place_data_df['STATE'] = place_data_df['STATE'].astype('str').str.pad(2,
                                                                              side='left',
                                                                              fillchar='0')
        place_data_df['PLACE'] = place_data_df['PLACE'].astype('str').str.pad(5,
                                                                              side='left',
                                                                              fillchar='0')
        place_data_df["Location"] = "geoId/" + place_data_df["STATE"] + place_data_df["PLACE"]

        cousub_data_df = data_df[(data_df['PLACE'] == 0) & (data_df['COUSUB'] != 0) &
                                 (data_df['SUMLEV'] == 61)]
        cousub_data_df['STATE'] = cousub_data_df['STATE'].astype('str').str.pad(2,
                                                                                side='left',
                                                                                fillchar='0')
        cousub_data_df['COUNTY'] = cousub_data_df['COUNTY'].astype('str').str.pad(3,
                                                                                  side='left',
                                                                                  fillchar='0')
        cousub_data_df['COUSUB'] = cousub_data_df['COUSUB'].astype('str').str.pad(5,
                                                                                  side='left',
                                                                                  fillchar='0')
        cousub_data_df["Location"] = "geoId/" + cousub_data_df["STATE"] + cousub_data_df[
            "COUNTY"] + cousub_data_df["COUSUB"]
        data_df = pd.concat([place_data_df, cousub_data_df], ignore_index=True)
        if 'sub-est2023.csv' in file_path:
            dup = data_df.copy()
            ...
            dup.to_csv("duplicate_place.csv", index=False, quotechar="'")
        ...
        if dt.now().year > 2023 and str(dt.now().year) in data_df.columns:
            final_cols.append(str(dt.now().year))
        data_df = data_df[final_cols]
        data_df.to_csv(f"loc_{file_name_without_ext}.csv", index=False)
        data_df = _unpivot_data_df(data_df, ["Location"], pop_cols)
...
def process(cleaned_csv_file_path: str, mcf_file_path: str, tmcf_file_path: str):
    """
    ...
    input_files += [
        os.path.join(_INPUT_FILE_PATH, file) for file in sorted(os.listdir(_INPUT_FILE_PATH))
    ]
    ...
    for file in input_files:
        ...
        data_df = pd.DataFrame()
        if 'co-est2021-pop' in file or 'co-est00int-01-51' in file:
            pass
