...
def _national_1900_1979(file_path: str):
    ...
    if int(year)<1960:
        if int(year)<1940:
            df = pd.read_csv(file_path,thousands=',',engine='python',skiprows=7,
                skipfooter=92,header=None)
        else:
            df = pd.read_csv(file_path,thousands=',',engine='python',skiprows=7,
                skipfooter=102,header=None)
        df.columns=['Age','Total','Count_Person_Male','Count_Person_Female',
        'White Total','White Male','White Female','NonWhite Total',
        'NonWhite Male','NonWhite Female']
        df = df.drop(columns=['Age','Total','White Total','White Male',
        ...
        df = pd.read_csv(file_path,thousands=',',engine='python',skiprows=6,
        skipfooter=102,header=None)
        df.columns=['Age','Total','Count_Person_Male','Count_Person_Female'
        ...
        'OtherRace Female']
        df = df.drop(columns=['Age','Total','White Total','White Male',
        'White Female','Black Total','Black Male','Black Female',
        'OtherRace Total','OtherRace Male','OtherRace Female'])
        df['Year'] = year
        df.insert(0, 'geo_ID', 'country/USA', True)
        df['Measurement_Method'] = 'dcAggregate/CensusPEPSurvey_PartialAggregate'
        return df
...
def _national_2010_2020(file_path: str):
    ...
    df.rename(columns = {'POPESTIMATE2010':'2010','POPESTIMATE2011':'2011',\
...
def _national_2021(file_path: str):
    ...
    df = df.query("AGE == 999")
    df = df.replace({'SEX':{1:'Count_Person_Male', 2:'Count_Person_Female'}})
    df.rename(columns = {'POPESTIMATE2021':'2021','SEX':'Year'},\
        inplace = True)
...
def _state_1970_1980(file_path: str):
    ...
    COLUMNS_TO_SUM = ['Under 5 years','5 to 9 years','10 to 14 years',\
    ...
    df=df.replace({'Race/Sex Indicator':{'White male':'Count_Person_Male',\
            'Black male':'Count_Person_Male','Other races male':'Count_Person_Male',
                'White female':'Count_Person_Female','Black female':\
                    'Count_Person_Female','Other races female':'Count_Person_Female'
        }})
...
def _state_1980_1990(file_path: str):
    year = file_path[-6:-4]
    year = 1900 + int(year)
    column_names = ['geo_ID','CountyCode','Age','Total',\
        'Count_Person_Male','Count_Person_Female']
    if year == 1987:
        df = pd.read_table(file_path,skiprows=29,\
            delim_whitespace=True,names=column_names)
    else:
        df = pd.read_table(file_path,skiprows=28,\
            delim_whitespace=True,names=column_names)
    df['geo_ID'] = 'geoId/' + (df['geo_ID'].map(str)).str.zfill(2)
    df['Year'] = year
...
def _state_2000_2010(file_path: str):
    geoid = file_path[-6:-4]
    column_name = ['AgeSex','April2000','2000','2001','2002','2003',\
        '2004','2005','2006','2007','2008','2009','April2010','2010']
    df = pd.read_csv(file_path,skiprows=4,thousands=',')
    df.columns = column_name
    df = df.query('AgeSex == "MALE" or AgeSex == "FEMALE"')
    df = df.replace({'AgeSex':{"MALE":'Count_Person_Male',\
        "FEMALE":'Count_Person_Female'}})
    df = df.drop(columns=['April2000','April2010','2010'])
    df = df.transpose().reset_index()
    df.columns = df.iloc[0]
...
def _state_2010_2020(file_path: str):
    ...
    df = df.replace({'YEAR':{1:'April2010Census',2:'April2010Estimate',\
        3:'2010',4:'2011',5:'2012',6:'2013',7:'2014',8:'2015',9:'2016',\
            10:'2017',11:'2018',12:'2019',13:'April2020',14:'2020'}})
    df = df.rename(columns = {'POPEST_MALE':'Count_Person_Male',\
        'POPEST_FEM':'Count_Person_Female','YEAR':'Year'})
    df = df.drop(columns=['SUMLEV','STATE','STNAME',
        'POPESTIMATE','UNDER5_TOT','UNDER5_MALE','UNDER5_FEM','AGE513_TOT',
        'AGE513_MALE','AGE513_FEM','AGE1417_TOT','AGE1417_MALE',
        'AGE1417_FEM','AGE1824_TOT','AGE1824_MALE','AGE1824_FEM',
        'AGE16PLUS_TOT','AGE16PLUS_MALE','AGE16PLUS_FEM','AGE18PLUS_TOT',
        'AGE18PLUS_MALE','AGE18PLUS_FEM','AGE1544_TOT','AGE1544_MALE',
        'AGE1544_FEM','AGE2544_TOT','AGE2544_MALE','AGE2544_FEM',
        'AGE4564_TOT','AGE4564_MALE','AGE4564_FEM','AGE65PLUS_TOT',
        'AGE65PLUS_MALE','AGE65PLUS_FEM','AGE04_TOT','AGE04_MALE',
        'AGE04_FEM','AGE59_TOT','AGE59_MALE','AGE59_FEM','AGE1014_TOT',
    ...
    df = df[(df['Year'] != 'April2010Census') & (df['Year']\
        != 'April2010Estimate') & (df['Year'] != 'April2020')]
    df['Measurement_Method'] = 'dcAggregate/CensusPEPSurvey_PartialAggregate'
    return df

def _state_2021(file_path: str):
    column_name = ['Age','April2020Total','April2020Male',
    'April2020Female','July2020Total','July2020Male',
    'July2020Female','2021Total','Count_Person_Male',
    'Count_Person_Female'] 
    ...
    df.insert(1,'geo_ID','geoId/'+geoid)
    df = df.drop(columns=['Age','April2020Total','April2020Male'
    ,'April2020Female','July2020Total','July2020Male',
    'July2020Female','2021Total'])
    df['Year'] = '2021'
    df['Measurement_Method'] = 'dcAggregate/CensusPEPSurvey_PartialAggregate'
    return df
...
def _county_1990_2000(file_path: str):
    ...
    df = df.replace({'Race-Sex':{1:'Count_Person_Male', 2:'Count_Person_Female'\
        ,3:'Count_Person_Male',4:'Count_Person_Female',5:'Count_Person_Male',\
            6:'Count_Person_Female',7:'Count_Person_Male',\
...
def _county_2000_2010(file_path: str):
    ...
    df = df.query('SEX != 0')
    df = df.replace({'SEX':{1:'Count_Person_Male', 2:'Count_Person_Female'}})
    df = df.drop(columns=['SUMLEV','STATE','COUNTY','STNAME','CTYNAME','AGEGRP',
    'ESTIMATESBASE2000','CENSUS2010POP','POPESTIMATE2010'])
    df.rename(columns = {'POPESTIMATE2000':'2000','POPESTIMATE2001':'2001',\
...
def _county_2010_2020(file_path: str):
    ...
    df = df.rename(columns = {'POPEST_MALE':'Count_Person_Male',\
        'POPEST_FEM':'Count_Person_Female','YEAR':'Year'})
    df = df.drop(columns=['SUMLEV','STATE','COUNTY','STNAME','CTYNAME',
        'POPESTIMATE','UNDER5_TOT','UNDER5_MALE','UNDER5_FEM','AGE513_TOT',
        'AGE513_MALE','AGE513_FEM','AGE1417_TOT','AGE1417_MALE',
        'AGE1417_FEM','AGE1824_TOT','AGE1824_MALE','AGE1824_FEM',
        'AGE16PLUS_TOT','AGE16PLUS_MALE','AGE16PLUS_FEM','AGE18PLUS_TOT',
...
def _county_2021(file_path: str):
    ...
    df['geo_ID'] = 'geoId/' + (df['STATE'].map(str)).str.zfill(2) +\
        + (df['COUNTY'].map(str)).str.zfill(3)
    df = df.replace({'YEAR':{1:'April2020Estimate',2:'July2020',\
        3:'2021'}})
    df = df.rename(columns = {'POPEST_MALE':'Count_Person_Male',\
        'POPEST_FEM':'Count_Person_Female','YEAR':'Year'})
    df = df.drop(columns=['SUMLEV','STATE','COUNTY','STNAME','CTYNAME',
        ...
        'AGE85PLUS_TOT','AGE85PLUS_MALE','AGE85PLUS_FEM',
        'MEDIAN_AGE_TOT','MEDIAN_AGE_MALE','MEDIAN_AGE_FEM'])
    df = df[(df['Year'] != 'April2020Estimate') & (df['Year']\
        != 'July2020')]
    df['Measurement_Method'] = 'CensusPEPSurvey'
    return df
...
class PopulationEstimateBySex:
    ...
    def process(self):
        ...
        final_df = _states_full_to_short_form(final_df, 'geo_ID',
                                         "geo_ID")
...
def main(_):
    ...
    loader = PopulationEstimateBySex(ip_files, cleaned_csv_path,\
        mcf_path, tmcf_path)
    loader.process()
