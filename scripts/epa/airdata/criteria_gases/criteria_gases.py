...
CSV_COLUMNS = [
    ...
    'Mean', 'Max', 'AQI', 'Units', 'Method']

# Template MCF for StatVarObservation
TEMPLATE_MCF = '''
Node: E:EPA_CriteriaGases_{pollutant}->E1
typeOf: dcs:StatVarObservation
variableMeasured: dcs:Mean_Concentration_AirPollutant_{suffix}
measurementMethod: C:EPA_CriteriaGases_{pollutant}->Method
observationDate: C:EPA_CriteriaGases_{pollutant}->Date
observationAbout: E:EPA_CriteriaGases_{pollutant}->E0
observationPeriod: dcs:"P1D"
value: C:EPA_CriteriaGases_{pollutant}->Mean
unit: C:EPA_CriteriaGases_{pollutant}->Units

Node: E:EPA_CriteriaGases_{pollutant}->E2
typeOf: dcs:StatVarObservation
...
observationAbout: E:EPA_CriteriaGases_{pollutant}->E0
observationPeriod: dcs:"P1D"
value: C:EPA_CriteriaGases_{pollutant}->Max
unit: C:EPA_CriteriaGases_{pollutant}->Units

Node: E:EPA_CriteriaGases_{pollutant}->E3
typeOf: dcs:StatVarObservation
...
observationAbout: E:EPA_CriteriaGases_{pollutant}->E0
observationPeriod: dcs:"P1D"
value: C:EPA_CriteriaGases_{pollutant}->AQI
'''

# Template MCF for Air Quality Site
TEMPLATE_MCF_AIR_QUALITY_SITE = '''
Node: E:EPA_CriteriaGases_{pollutant}->E0
typeOf: dcs:AirQualitySite
dcid: C:EPA_CriteriaGases_{pollutant}->Site_Number
name: C:EPA_CriteriaGases_{pollutant}->Site_Name
location: C:EPA_CriteriaGases_{pollutant}->Site_Location
containedInPlace: C:EPA_CriteriaGases_{pollutant}->County
'''
    ...
    with open(tmcf_file_path, 'w') as f_out:   
        ...
        f_out.write(
            TEMPLATE_MCF.format_map({
                'pollutant': pollutant,
                'suffix': get_suffix(pollutant)
            }))


if __name__ == '__main__':
    pollutant = sys.argv[1]
    create_csv(f'EPA_CriteriaGases_{pollutant}.csv')
            ...
            if filename.endswith('.zip'):
                ...
                with ZipFile(dirpath + os.sep + filename) as zf:
                    with zf.open(filename[:-4] + '.csv', 'r') as infile:
                        ...
                        write_csv(f'EPA_CriteriaGases_{pollutant}.csv', reader)
