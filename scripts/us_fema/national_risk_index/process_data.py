...
FLAGS = flags.FLAGS
flags.DEFINE_string('output_path', 'gs://unresolved_mcf/us_fema/national_risk_index/latest/', 'The local path to download the files')
...
def convert_month_year(date_str):
  ...
  """
  try:
    date_obj = datetime.strptime(date_str, '%B %Y')  # Parse the input string
...
def main(argv):
    ...
    for input_path in INPUT_TO_OUTPUT_PATHS:
        ...
        output_path_new =FLAGS.output_path.rstrip("/") + "/" + out_put_file_name
        process_csv(input_path, output_path_new, "output/csv_columns.json", str(out_put_file_name))
