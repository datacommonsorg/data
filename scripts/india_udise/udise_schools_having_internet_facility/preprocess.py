...
"""Classes and methods to import Number of Schools having Internet Facility from Unified District Information System for Education (UDISE)"""
...
class UDISESchoolsHavingInternetFacility(UDISEIndiaSchoolDataLoaderBase):

    def _get_base_name(self, data_row):
        name = "Count_School_HasInternet"
        return name

    def _get_base_constraints(self, data_row):
        constraints = []
        constraints.append("amenityFeature: dcs:Internet")
        return constraints
...
if __name__ == "__main__":
    ...
    csv_file_path = os.path.join(
        module_dir_, "UDISEIndia_Schools_Having_Internet_Facility.csv")
    mcf_file_path = os.path.join(
        module_dir_, "UDISEIndia_Schools_Having_Internet_Facility.mcf")
