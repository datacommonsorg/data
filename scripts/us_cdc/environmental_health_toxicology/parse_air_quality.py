...
def generate_county_dcid(data):
    ...
    for index, row in data.iterrows():
        list_dcids.append("geoId/" + str(row["statefips"]).zfill(2)\
         + str(row["countyfips"]).zfill(3))
    return(list_dcids)

def generate_date(data):
    list_dates = []
    for index, date in data.items():
        day = date[:2]
...
def clean_census_tract_data(input_file):
...
def clean_county_data(input_file):
...
def clean_air_quality_data(input_file, output_file):
    ...
    if "Census" in input_file:
        data = clean_census_tract_data(input_file)
    elif "County" in input_file:
        data = clean_county_data(input_file)
