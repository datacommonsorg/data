...
FLAGS = flags.FLAGS
flags.DEFINE_string('nors_input_path', './data/NationalOutbreakPublicDataTool.xlsx', '')
flags.DEFINE_string('nors_sheet_name', 'Outbreak Data', '')
flags.DEFINE_string('nors_schema_map', './data/col_map_exc_food.json', '')
flags.DEFINE_string('nors_output_path', './data/output', 'Path to the directory where the generated files are saved')
...
def generate_aggregates(df:pd.DataFrame, groupby_cols:list=None)->pd.DataFrame:
    ...
    country_stat_df = country_stat_df.groupby(groupby_cols, as_index=False).agg({'Illnesses': sum, 'Hospitalizations': sum, 'Deaths': sum})

    # aggreagte stats for US states and territories
    aggregate_df = df[~(df['observationAbout'].str.contains('country'))]
    aggregate_df = aggregate_df.groupby(groupby_cols, as_index=False).agg({'Illnesses': sum, 'Hospitalizations': sum, 'Deaths': sum})

    # aggregate for US at country level
    country_group_by = [cols for cols in groupby_cols if cols != 'observationAbout']
    us_aggreagte_df = aggregate_df.groupby(country_group_by, as_index=False).agg({'Illnesses': sum, 'Hospitalizations': sum, 'Deaths': sum})
    us_aggreagte_df['observationAbout'] = 'country/USA'
    us_aggreagte_df = us_aggreagte_df[['observationDate', 'observationAbout', 'Primary Mode', 'Illnesses', 'Hospitalizations', 'Deaths']]
    ...
    if pd.notna(row['Etiology']) and row['Etiology'] != '':
        ...
        if len(etiology_list) == 1: 
            for etiology in etiology_list:
                if 'etiology' not in sv_dict.keys():
                   sv_dict.update(PV_MAP['Etiology'][etiology])
                    ...
                    sv_dict['etiology'] = sv_dict['etiology'] + '__' + PV_MAP['Etiology'][etiology]['etiology']
            ...
            if pd.notna(row['Etiology Status']):
                ...
                assert len(etiology_status_list) == len(etiology_list), "Etiology and Etiology Status have varying length, please check."
                for status in etiology_status_list:
                    if 'medicalStatus' not in sv_dict.keys():
                        sv_dict.update(PV_MAP['Etiology Status'][status])
                    else:
                        sv_dict['medicalStatus'] = sv_dict['medicalStatus'] + '__' + PV_MAP['Etiology Status'][status]['medicalStatus']
...
def fix_place_names(clean_df):
    ...
    clean_df = clean_df[~(clean_df['observationAbout'] == '')] # remove MultiState
    clean_df.drop(columns=['State'], inplace=True)
    return clean_df

def process_non_infectious_data(input_path:str, sheet_name:str, schema_map_path:str,output_path:str)->None:
    ...
    trans_mode_df = generate_aggregates(df, groupby_cols=['observationDate', 'observationAbout', 'Primary Mode'])
    trans_mode_df = trans_mode_df.drop_duplicates(subset=['observationDate', 'observationAbout', 'Primary Mode'], keep='last')
    trans_mode_df['agg_by'] = 'Transmission Mode'


    etiology_df = generate_aggregates(df, groupby_cols=['observationDate', 'observationAbout', 'Primary Mode', 'Etiology'])
    ...
    sv_df = sv_df[["variable", "Primary Mode", "Etiology", "Etiology Status", "variableMeasured"]]

    # prepare the final clean csv
    clean_df = pd.merge(clean_df, sv_df, left_on=['variable', 'Primary Mode', 'Etiology', 'Etiology Status'], right_on=['variable', 'Primary Mode', 'Etiology', 'Etiology Status'], how='left')
...
def main(_)->None:
    process_non_infectious_data(FLAGS.nors_input_path, FLAGS.nors_sheet_name, FLAGS.nors_schema_map, FLAGS.nors_output_path)
