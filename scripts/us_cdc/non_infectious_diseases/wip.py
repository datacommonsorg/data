...
_STAT_COLS = ['Illnesses', 'Hospitalizations', 'Info on Hospitalizations', 'Deaths', 'Info on Deaths']
...
def generate_aggregates(df:pd.DataFrame, groupby_cols:list=None)->pd.DataFrame:
    ...
    aggregate_df = aggregate_df.groupby(groupby_cols, as_index=False).agg({'Illnesses': sum, 'Hospitalizations': sum, 'Info on Hospitalizations': sum, 'Deaths': sum, 'Info on Deaths': sum})
...
def make_stat_vars(row, PV_MAP):
    ...
    sv_name_dict = {
        'about' : row['variable'],
        'pop_type': sv_dict['populationType'],
        'mprop': sv_dict['measuredProperty']
    }
    ...
    if pd.notna(row['Primary Mode']) and row['Primary Mode'] != '':
        ...
        sv_name_dict['trans_mode'] = row['Primary Mode']
    ...
    if pd.notna(row['Etiology']) and row['Etiology'] != '':
        ...
        if len(etiology_list) == 1: 
            sv_name_dict['etiology'] = etiology_list
            ...
            if pd.notna(row['Etiology Status']):
                ...
                sv_name_dict['etio_stat'] = etiology_status_list
            else:
                sv_name_dict['etio_stat'] = ''
