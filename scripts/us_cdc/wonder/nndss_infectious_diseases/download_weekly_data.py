...
_WEEKLY_TABLE_2017 = _BASE_URL + 'nndss_weekly_tables_menu.asp?savedmode=&mmwr_year={year}&mmwr_week={week}'
_FILENAME_TEMPLATE = "mmwr_year_{year}_mmwr_week_{week}_mmwr_table_{id}"
_BAD_URLS = ['https://wonder.cdc.gov/nndss/nndss_weekly_tables_1995_2014.asp?mmwr_year=2007&mmwr_week=13&mmwr_table=1&request=Submit']
...
def extract_table_from_link(table_url:str,filename:str, output_path:str, update:bool=False)->None:
	"""
	"""
	num_tries = 10
...
def scrape_table_links_from_page(page_url:str, output_path:str, update:bool=False)->None:
	"""
	"""
	...
	for table_link in table_link_list:
		...
		if table_link.endswith('.html') and 'table' in table_link:
			...
			filename_components = table_link.split('/')[-1].split('.html')[0].split('-')
			filename = _FILENAME_TEMPLATE.format(year=filename_components[0], week=filename_components[1], id=filename_components[2].split('table')[1])
			extract_table_from_link(table_url, filename, output_path, update)
...
def download_weekly_nnds_data_across_years(year_range:str, output_path:str)->None:
	"""
	"""
