...
def clean_precipitation_data(file_path, output_file):
    ...
    data["observationDate"] = data["year"].astype(str) + "-" + data["month"].astype(str)
    ...
    if "Evapotranspiration" in file_path:
        ...
        data = pd.melt(
            data,
            id_vars=['state', 'county', 'fips', 'year', 'month', 'observationDate'],
            value_vars=[
                "StandardizedPrecipitation" + "EvapotranspirationIndex"
            ],
            var_name='variableMeasured',
            value_name='value')
        data["observationAbout"] = "geoId/" + data["fips"].astype(str)
        ...
        data = pd.melt(
            data,
            id_vars=['year', 'month', 'observationDate', 'statefips', 'countyfips'],
            value_vars=["PalmerDroughtSeverityIndex"],
            var_name='variableMeasured',
            value_name='value')
        data["observationAbout"] = "geoId/" + data["countyfips"].astype(str)
    else:
    ...
    # Add "dcid:" prefix to the specified columns
    data['observationAbout'] = "dcid:" + data['observationAbout'].astype(str)
    data['variableMeasured'] = "dcid:" + data['variableMeasured'].astype(str)
