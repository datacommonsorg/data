...
def rename_columns(df_column_list):
    try:
        logging.info(f"Renaming columns based on mapping")
        ...
        logging.fatal(f"Error during column renaming: {e}")


def make_name_str(column_components, disease_match, dcid):
    try:
        logging.info(f"Generating name string.")
        pv_str = []
        age_pv = ''
        # collect all pvs
        ...
        logging.fatal(f"Error creating name string: {e}")


def map_pvs_from_column_name(row):
    try:
        logging.info(f"Mapping PVs processing.")
        row['sv_dcid'] = ''
        row['sv_dict'] = {}
        ...
        if 'medicalCondition' in sv_dict.keys():
            sv_dict['medicalCondition'] = sv_dict[
                'medicalCondition'] + "__" + row['dcid']
        else:
        ...
        logging.fatal(f"Error mapping PVs: {e}")


def clean_single_dataframe(df, year, table):
    try:
        logging.info(f"Cleaning DataFrame for year: {year}, table: {table}")
        ...
        diseases_map = _DISEASE_DCID_DF[
            (_DISEASE_DCID_DF['table'] == int(table)) &
        ...
        diseases_map['variable'] = diseases_map[
            'Replacement'] + ';' + diseases_map['variable']
        ...
        diseases_map = diseases_map[diseases_map['value'].str.contains(
            '\\d', regex=True)]

        diseases_map.rename(columns={'year': 'observationDate'}, inplace=True)
        diseases_map['observationDate'] = diseases_map[
            'observationDate'].astype(int)
        ...
        logging.info(
            f"Data cleaning for year: {year}, table: {table} completed.")
        return diseases_map, sv_df
    except Exception as e:
        logging.fatal(
            f"Error cleaning DataFrame for year: {year}, table: {table}. Error: {e}"
        )
...
def generate_statvars(full_sv_df):
    try:
        ...
        logging.fatal(f"Error generating StatVars: {e}")


def write_svdicts_to_file(dict_list, file_path):
    try:
        logging.info(f"Writing {len(dict_list)} sv dict to a file: {file_path}")
        unique_dict = [
        ...
        for udict in unique_dict:
            ...
            f.write('\n')
        f.close()
        logging.info(f"SVDictionaries written successfully to: {file_path}")
    except Exception as e:
        logging.fatal(
            f"Error writing dictionaries to file: {file_path}. Error: {e}")
