...
# Reading and concatinating old File from BQ to get the remaining SVs
df_old = pd.read_csv("OECD_life_expectancy_and_mortality_cleaned_BQ.csv")
df_old["observation_about"] = "dcid:" + df_old["observation_about"]
# Pivoting the file to have the same format for both dataframes
df_pivoted = df_old.pivot_table(index=["observation_about", "observation_date"],
                                columns="variable_measured",
                                values="value")

# Making sure to have same set of key column names for concat
df_cleaned_bq = df_pivoted.reset_index()
df_cleaned_bq = df_cleaned_bq.rename(columns={
    'observation_about': 'Region',
    'observation_date': 'Year'
})
df_cleaned_bq['Year'] = '"' + df_cleaned_bq['Year'].astype(str) + '"'
df_cleaned_bq.to_csv("BQ_Final.csv", index=False)
# Concatinating old and new csv to retain old SVs

merged_df = df_cleaned_bq.merge(df_cleaned, on=['Region', 'Year'], how='outer')
merged_df.to_csv('OECD_life_expectancy_and_mortality_cleanedN.csv',
                 index=False,
