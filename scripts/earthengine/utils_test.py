  ...
  def test_dict_filter_values(self):
    ...
    self.assertFalse(
        utils.dict_filter_values(
            {'param1': ['value1', 'drop this']}, filter_config
        )
    )
    self.assertTrue(
        utils.dict_filter_values(
            {'param1': 'allowed', 'param2': 'do not drop this'}, filter_config
        )
    )
...
class S2CellUtilsTest(unittest.TestCase):
  ...
  def test_s2_cell_area(self):
    self.assertTrue(
        math.isclose(
            53.23, utils.s2_cell_area(0x0000050000000000), rel_tol=1e-2
        )
    )
    self.assertTrue(
        math.isclose(
...
class GridCellUtilsTest(unittest.TestCase):
  ...
  def test_grid_id_from_latlng(self):
    self.assertEqual('dcid:grid_1/20_30', utils.grid_id_from_lat_lng(1, 20, 30))
  ...
  def test_grid_ids_distance(self):
    ...
    self.assertTrue(
        math.isclose(
            143.84,
            utils.grid_ids_distance(
                'ipcc_50/34.25_-85.25_USA', 'ipcc_50/35.25_-84.25_USA'
            ),
            rel_tol=1e-2,
        )
    )
  ...
  def test_grid_area(self):
    self.assertTrue(
        math.isclose(
            10248.33, utils.grid_area('dcid:grid_1/34_-85'), rel_tol=1e-2
        )
    )
    self.assertTrue(
        math.isclose(
            2554.56, utils.grid_area('ipcc_50/34.25_-85.25_USA'), rel_tol=1e-2
        )
    )
...
class StrUtilsTest(unittest.TestCase):
  ...
  def test_strip_namespace(self):
    self.assertEqual('country/IND', utils.strip_namespace('dcid:country/IND'))
    self.assertEqual(
  ...
  def test_str_get_numeric_vlaue(self):
    ...
    self.assertEqual(6543210.12, utils.str_get_numeric_value(' 6,543,210.12 '))
...
class DateUtilsTest(unittest.TestCase):
  ...
  def test_date_advance_by_time_period(self):
    ...
    self.assertEqual(
        '2023-04-10', utils.date_advance_by_period('2023-04-20', 'P-10D')
    )
    self.assertEqual(
        '2023-06', utils.date_advance_by_period('2023-04', 'P2M', '%Y-%m')
    )
    self.assertEqual('2033', utils.date_advance_by_period('2023', '10Y', '%Y'))

  def test_date_format_by_time_period(self):
    self.assertEqual(
        '2023-04-10', utils.date_format_by_time_period('2023-04-10', 'P1D')
    )
    self.assertEqual(
        '2022-04', utils.date_format_by_time_period('2022-04-10', 'P3M')
    )
    self.assertEqual(
        '2021', utils.date_format_by_time_period('2021-01-10', '1Y')
    )
