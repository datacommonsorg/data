...
def s2_cells_distance(cell_id1: int, cell_id2: int) -> float:
  ...
  return distance.distance(
      (p1.lat().degrees, p1.lng().degrees), (p2.lat().degrees, p2.lng().degrees)
  ).km
...
def latlng_cell_area(
    lat: float, lng: float, height: float, width: float
) -> float:
...
def grid_id_to_deg_lat_lng(
    grid_id: str, default_deg: float = 1
...
def place_id_to_lat_lng(
    placeid: str, dc_api_lookup: bool = True
) -> (float, float):
...
def date_advance_by_period(
    date_str: str, time_period: str, date_format: str = '%Y-%m-%d'
) -> str:
