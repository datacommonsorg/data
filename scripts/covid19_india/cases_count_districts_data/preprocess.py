...
def _get_district_code(district):
    ...
    if district not in ['Unknown']:
        close_match = difflib.get_close_matches(str(district).upper(), dist_code['DistrictName(InEnglish)'],
                            n=1, cutoff=0.8)
        if close_match:
            return dist_code[dist_code['DistrictName(InEnglish)'] == close_match[0]]['DistrictCode'].values[0]
        else:
            return None
...
def create_formatted_csv_file(csv_file_path, df):
    ...
    dist_codes_dict = dict(zip(df['District'].unique().tolist(), district_codes))
