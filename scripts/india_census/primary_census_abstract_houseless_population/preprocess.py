...
class CensusPrimaryCensusAbstractHouselessPopulationDataLoader(
    ...
    def _get_base_name(self, row):
        name = "Count_" + row["populationType"] + "_" + "Houseless"
        return name

    def _get_base_constraints(self, row):
        constraints = "householdType: dcs:Houseless"
        return constraints
...
if __name__ == '__main__':
    ...
    mcf_file_path = os.path.join(
        os.path.dirname(__file__),
        './IndiaCensus2011_Primary_Abstract_HouselessPopulation.mcf')
    tmcf_file_path = os.path.join(
        os.path.dirname(__file__),
        './IndiaCensus2011_Primary_Abstract_HouselessPopulation.tmcf')

    csv_file_path = os.path.join(
        os.path.dirname(__file__),
        './IndiaCensus2011_Primary_Abstract_HouselessPopulation.csv')
    loader = CensusPrimaryCensusAbstractHouselessPopulationDataLoader(
        ...
        census_year=2011,
        dataset_name="Primary_Abstract_HouselessPopulation")
    loader.process()
