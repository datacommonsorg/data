...
_CITY_MAP = {
    'monroe township nj': 'monroe township gloucester county nj'
}
...
def read_places(places, cities_all, place_type):
  ...
  for place in places:
    ...
    count_places += 1
    cities_all[cn] = place
  logging.info('Total places {}, ignore {}, ignore name {}, ignore funcstat {}'.format(
...
def construct_obs(year, crime_type, geo_id, value):
  return _OBS_TEMPLATE.format(
      year=year, crime=crime_type, geo=geo_id, value=value)

def write_mcf(geocode, crime_type, value, year, mcf_output_f,
              mcf_names):
  ...
  mcf_output_f.write(
      construct_obs(
          geo_id=geocode, crime_type=crime_type, value=value, year=year))
...
def compute_totals(r):
  ...
  if violent_computed != violent:
    print('{} {} {} violent mismatch {} {}'.format(
        r['Year'], r['City'], r['State'],
      ...
      with open('fbi.mcf', 'w') as mcf_output_f:
        ...
        for crime in crimes:
            try:
                ...
                logging.error('{} state not found {}'.format(crime['State'], lookup_name))
                continue
            ...
            for k, value in crime.items():
                ...
                if crime[k] != '':
                    write_mcf(geocode, k, value, year, mcf_output_f, mcf_names)
                    count_pop_obs += 1
                else:
                    print('No obs for {} {} {}'.format(
                        year, city_state, k))
                    count_no_obs += 1
