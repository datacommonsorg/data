...
class HateCrimeTable4Test(unittest.TestCase):

    def test_csv(self):
        csv_files = []
        test_config = {
            'type': 'xls',
            'path': 'testdata/2017.xls',
            'args': {
                'header': 5,
                'skipfooter': 3
            }
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            csv_file_path = os.path.join(tmp_dir, '2017.csv')

            read_file = pd.read_excel(xls_file_path, **test_config['args'])
            read_file = preprocess._clean_dataframe(read_file, '2017')
            read_file.insert(_YEAR_INDEX, 'Year', '2017')
