...
class HateCrimeTable12Test(unittest.TestCase):

    def test_csv(self):
        csv_files = []
        test_config = {
            'type': 'xls',
            'path': 'testdata/2010.xls',
            'args': {
                'header': 2
            }
        }
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            csv_file_path = os.path.join(tmp_dir, '2010.csv')

            read_file = pd.read_excel(xls_file_path,
                                      **test_config['args'],
                                      usecols=[0, 1, 2, 3, 4])
            read_file = preprocess._clean_dataframe(read_file, '2010')
            read_file.insert(_YEAR_INDEX, 'Year', '2010')
