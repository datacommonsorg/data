...
class HateCrimeTable9Test(unittest.TestCase):

    def test_csv(self):
        csv_files = []
        test_config = {
            'type': 'xls',
            'path': 'testdata/2012.xls',
            'args': {
                'header': 2,
                'skipfooter': 2,
                'usecols': [0, 1]
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            csv_file_path = os.path.join(tmp_dir, '2012.csv')

            read_file = pd.read_excel(xls_file_path, **test_config['args'])
            read_file = preprocess._clean_dataframe(read_file, '2012')
            read_file.insert(_YEAR_INDEX, 'Year', '2012')
