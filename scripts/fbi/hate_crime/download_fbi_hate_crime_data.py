...
def download_hate_crime_data(json_api_url, output_folder):
    ...
    try:
        ...
        logging.fatal(
            f"An unexpected error occurred while fetching/parsing initial JSON: {e}"
        )
    ...
    try:
        ...
        logging.error(
            f"Error downloading the ZIP file from {extracted_zip_url}: {e}")
    ...
    try:
        ...
        for root, dirs, files in os.walk(temp_extract_folder):
            for file in files:
                ...
                if file.lower().endswith('.csv'):
                    ...
                    target_csv_path = os.path.join(output_folder,
        ...
        if os.path.exists(temp_extract_folder):
            ...
            print(
                f"Cleaned up temporary extraction folder: {temp_extract_folder}"
            )
