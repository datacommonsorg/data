...
_US_ALPHA_CODE_UPDATE_MAP = {
    # Mapping state alpha codes in data to geos in csv / map
    'NB': 'NE',
    'GM': 'GU'
}
...
with open(os.path.join(_CITY_GEOCODES_PATH), encoding='utf8') as csvfile:
    ...
    for a in csv_reader:
        if _CITY.get(a[0], ''):  # if key already exists in _CITY
            assert False, f'Duplicate city found {a}'
        _CITY[a[0]] = a[1]
...
def _state_to_dcid(state_code: str) -> str:
    ...
    # if state_code is in the map, return it's value else retain the original
    state_code = _US_ALPHA_CODE_UPDATE_MAP.get(state_code, state_code)

    return USSTATE_MAP.get(state_code, '')
...
def _county_to_dcid(state_abbr: str, county: str) -> str:
    ...
    # if state_abbr is in the map, return it's value else retain the original
    state_abbr = _US_ALPHA_CODE_UPDATE_MAP.get(state_abbr, state_abbr)

    county_valid_names = _get_county_variants(county)

    if state_abbr in COUNTY_MAP:
        for c in county_valid_names:
            if c in COUNTY_MAP[state_abbr]:
                return COUNTY_MAP[state_abbr][c]
...
def convert_to_place_dcid(state_abbr: str,
    ...
    if geo_type == 'State':
        geo_dcid = _state_to_dcid(state_abbr)

    elif geo_type == 'County':
        geo_dcid = _county_to_dcid(state_abbr, geo)

    elif geo_type == 'City':
        geo_dcid = _city_to_dcid(state_abbr, geo)
