...
class CBPMSAProcessor:
  ...
  def __init__(self, input_file_obj, output_dir, year, is_test_run=False):
    ...
    # Construct CSV file name andMCF FIle name
    self.csv_output_filename = f'cbp_{self.year}_msa.csv'
    self.csv_output_path = os.path.join(self.output_dir, self.csv_output_filename)
    self.mcf_output_filename = f'cbp_{self.year}_msa.mcf'
    self.mcf_output_path = os.path.join(self.output_dir, self.mcf_output_filename)
    ...
    try:
      with open(self.csv_output_path, 'w', newline='') as csv_f, \
        ...
        for r in rows:
          ...
          csv_writer.writerow([
              geoid, self.year, naics, est, emp, emp_nf, annual_payroll, ap_nf
          ])

          # Generate and write MCF data
          pop_est, obs_est, pop_emp, obs_emp, obs_payroll = write_mcf.mcf_for(
              geoid, self.year, naics, est=est, emp=emp, ap=annual_payroll)

          pops, obs = write_mcf.write_mcf_pop_obs(
              pop_est, obs_est, emp_nf, pop_emp, obs_emp, ap_nf, obs_payroll,
              mcf_f)
          self.count_pops += pops
          self.count_obs += obs

      logging.info('CBP data processing complete for year %s.', self.year)
      logging.info('Wrote %d pops, %d obs', self.count_pops, self.count_obs)
