...
import time
import shutil
from util import file_util as fileutil
import re
...
def write_aggregate_csv(year, input_dir, out_dir):
    ...
    # except Exception as e:
    #     logging.info(f"Error in write_aggregate_csv for year {year}: {e}")
...
def fetch_and_write(county_name, year, svs, input_dir):
    ...
    try:
        ...
        # else:
        #     logging.info(f"No data to write for county {county_name}. Skipping file creation.")
        # logging.info(
        #     f"Writing {len(county_csv_rows)} rows for county {county_name} to file {out_file}")
    except Exception as e:
...
def get_survey_county_data(year, county, input_dir):
    ...
    if _FLAGS.mode == "" or _FLAGS.mode == "download":
        ...
        if os.path.exists(response_file):
            logging.info(f"File exist for {county} - {year}")
            return None
        params = {
...
def get_data(params):
    ...
    logging.info(f"Fetching data from API: {params['county_name']}")
    ...
    try:
        ...
        if response.status_code != 200:
            logging.warning(
                f"API returned non-200 status code: {response.status_code} for county {params['county_name']}. Retrying..."
            )
