...
flags.DEFINE_string(
    'output_dir', 'output', 'Output firectory for generated files.'
)
...
def write_aggregate_csv(year, out_dir):
  ...
  with open(out_file, 'w', newline='') as out:
    csv_writer = csv.DictWriter(
        out, fieldnames=CSV_COLUMNS, lineterminator='\n'
    )
...
def to_csv_row(data_row, svs):
  ...
  if (
      data_row['domaincat_desc']
      and data_row['domaincat_desc'] != 'NOT SPECIFIED'
  ):
  ...
  value = (
      (data_row['value'] if 'value' in data_row else data_row['Value'])
      .strip()
      .replace(',', '')
  )
  ...
  observation_about = (
      ...
      if data_row['state_fips_code']
      else 'dcid:country/USA'
  )
