...
def process_survey_data(year, svs, out_dir):
  ...
  with multiprocessing.Pool(pool_size) as pool:
    pool.starmap(fetch_and_write, zip(county_names, repeat(year), repeat(svs),
                                      repeat(out_dir)))
...
def write_aggregate_csv(year, out_dir):
  ...
  with open(out_file, 'w', newline='') as out:
    csv_writer = csv.DictWriter(out, fieldnames=CSV_COLUMNS,
                                lineterminator='\n')
...
def to_csv_row(data_row, svs):
  ...
  value = (data_row['value'] if 'value' in data_row else data_row[
    'Value']).strip().replace(',', '')
