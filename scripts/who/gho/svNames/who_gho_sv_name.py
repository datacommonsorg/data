...
def get_name(i_name, dcid, i_code, name_replacements, dup_i_names):
  ...
  additional_values_name = ''.join(' ' + c if c.isupper() else c for c in additional_values)
  ...
  if i_name in dup_i_names:
    name = i_name.title() + " (" + i_code + ")" + additional_values_name.title()
  return name
  ...
  for line in mcf_file_lines:
    ...
    if line == '\n':
      ...
      if not i_name:
        ...
        continue
      name = get_name(i_name, dcid, i_code, name_replacements, dup_i_names)
