...
def preprocess(input_table_path, existing_facilities_file):
    ...
    """
    logging.info(f"Starting `preprocess` function. Input path: {input_table_path}")
    ...
    logging.info(f"Number records (from existing_facilities_path): {len(pd.read_csv(existing_facilities_path))}")
...
def main(_):
    ...
    existing_facilities_full_path = os.path.join(FLAGS.input_download_path, FLAGS.existing_facilities_file + ".csv")
