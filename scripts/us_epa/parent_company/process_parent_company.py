...
def _batch_get_counties(zip_codes: set) -> dict:
    """
    ...
    for zip_dcid in zip_dcids:
        ...
        # First, try to find a county from the geoWithin data
        if zip_dcid in geo_within_data:
            for place in geo_within_data[zip_dcid]:
                # Handle cases where place can be a dict or a string
                type_of_place = ""
...
def process_companies(input_table_path, existing_facilities_file,
    ...
    It reads the input file once into memory to improve performance.
    """
    ...
    # --- Pass 1: Read data into memory and get all zip codes ---
    logging.info(f"Reading all data from {input_table_csv_path} into memory...")
    all_rows = []
    all_zip_codes = set()
    try:
        with open(input_table_csv_path, "r", newline='') as rfp:
            cr = csv.DictReader(rfp)
            zip_col = next(
                (f for f in cr.fieldnames if f.lower() == 'parent_co_zip'),
                None)
            all_rows = list(cr)  # Read all rows into a list of dicts
        ...
        logging.fatal(f"Input file not found: {input_table_csv_path}")
        sys.exit(1)

    logging.info(f"Read {len(all_rows)} rows into memory.")

    # --- Batch fetch all counties ---
    ...
    with \
        ...
        if not all_rows:
            ...
            actual_csv_headers = {
                field.lower(): field for field in all_rows[0].keys()
            }
            ...
            if not all([
                ...
                logging.fatal(
                    ...
                    f"Headers: {all_rows[0].keys()}. Script cannot proceed.")
                sys.exit(1)

            logging.info(
                f"Using resolved CSV column mappings for processing rows. "
            )

            for i, in_row in enumerate(all_rows):
                if i % 1000 == 0:
                    logging.info(
                        f"Processing row {i} from memory (generating output)."
                    )
...
def generate_svobs_helper(ownership_relationships_filepath, svobs_path_info):
    ...
    for i in range(0, len(facilities), batch_size):
        ...
        try:
            ...
            facility_svo_dict.update(svo_batch)
    ...
    logging.info(f"# SVs : (processed in batches)")
