...
def _add_to_duplicate_mapping(company_id_1, company_id_2, company_id_count):
    if ((company_id_1 in _DUPLICATE_MAPPING and
        ...
        logging.debug(
            f"Skipping duplicate mapping already present: {company_id_1} <-> {company_id_2}"
        )
        return
...
def _run_deduplication_preprocessing_steps(input_table_path,
                                           existing_facilities_file):
    """
    ...
    with open(input_table_csv_path, "r", newline='') as rfp:
        ...
        logging.info(
            f"CSV Headers found in '{input_table_csv_path}': {cr.fieldnames}")
        ...
        CSV_PARENT_COMPANY_NAME_COL = actual_csv_headers.get(
            'parent_company_name')
        CSV_YEAR_COL = actual_csv_headers.get('year')
        CSV_PARENT_CO_PERCENT_OWN_COL = actual_csv_headers.get(
            'parent_co_percent_own')
        CSV_PARENT_CO_STREET_ADDRESS_COL = actual_csv_headers.get(
            'parent_co_street_address')
        ...
        if not all([
                CSV_FACILITY_ID_COL, CSV_PARENT_COMPANY_NAME_COL, CSV_YEAR_COL,
                CSV_PARENT_CO_PERCENT_OWN_COL, CSV_PARENT_CO_STREET_ADDRESS_COL,
                CSV_PARENT_CO_CITY_COL, CSV_PARENT_CO_STATE_COL,
                CSV_PARENT_CO_ZIP_COL
        ]):
            logging.fatal(
        ...
        for i, in_row in enumerate(cr):
            if i % 500 == 0:
                logging.info(
                    f"Preprocessing row {i} to build deduplication maps...")

            facility_id = in_row.get(CSV_FACILITY_ID_COL, '').strip()
            if not facility_id:
                logging.warning(
                    f"⚠️ Skipping preprocessing of row {i+1}: Mandatory FACILITY_ID is empty or blank. Row: {in_row}"
                )
            ...
            company_name_raw = in_row.get(CSV_PARENT_COMPANY_NAME_COL,
                                          '').strip()
            company_name = company_name_raw.replace("\"", "").replace("'", "")
            if not company_name:
                logging.warning(
                    f"⚠️ Skipping preprocessing of row {i+1}: 'PARENT_COMPANY_NAME' not found or empty for {ghg_id}. Row: {in_row}"
                )
            ...
            if not company_id:
                logging.warning(
                    f"⚠️ Skipping preprocessing of row {i+1}: Mandatory company_id is empty (from name_to_id) for {company_name}. Row: {in_row}"
                )
    ...
    for k, v in _COUNTERS_COMPANIES.items():
        if isinstance(v, set) and v:
            ...
            logging.info(
                f"{k}: 0 entries or non-set type (or handled elsewhere).")
...
def process_companies(input_table_path, existing_facilities_file,
    ...
    with \
        ...
        with open(input_table_csv_path, "r", newline='') as rfp:
            ...
            actual_csv_headers = {
                field.lower(): field for field in cr.fieldnames
            }
            ...
            if not all([
                    CSV_FACILITY_ID_COL, CSV_PARENT_COMPANY_NAME_COL,
                    CSV_YEAR_COL, CSV_PARENT_CO_PERCENT_OWN_COL,
                    CSV_PARENT_CO_STREET_ADDRESS_COL, CSV_PARENT_CO_CITY_COL,
                    CSV_PARENT_CO_STATE_COL, CSV_PARENT_CO_ZIP_COL
            ]):
                logging.fatal(
                    f"❌ FATAL: One or more critical CSV columns not found in main processing loop. "
            ...
            for i, in_row in enumerate(cr):
                if i % 1000 == 0:
                    logging.info(
                        f"Processing row {i} from main input table (generating output)."
                    )

                facility_id = in_row.get(CSV_FACILITY_ID_COL, '').strip()
                if not facility_id:
                    logging.warning(
                        f"⚠️ Skipping row {i+1} (output generation): Mandatory FACILITY_ID is empty. Row: {in_row}"
                    )
                    _COUNTERS_COMPANIES["facility_id_extraction_failed"].add(
                        str(in_row))
                    continue
                ...
                if not company_name:
                    logging.warning(
                        f"⚠️ Skipping row {i+1} (output generation): 'PARENT_COMPANY_NAME' not found or empty for {ghg_id}. Row: {in_row}"
                    )
                    _COUNTERS_COMPANIES["company_name_not_found"].add(ghg_id)
                    continue
                ...
                if not company_id:
                    logging.warning(
                        f"⚠️ Skipping row {i+1} (output generation): Mandatory company_id is empty (from name_to_id) for {company_name}. Row: {in_row}"
                    )
                    _COUNTERS_COMPANIES["company_id_name_to_id_failed"].add(
                        str(in_row))
                    continue
                ...
                if not year:
                    logging.warning(
                        f"⚠️ Skipping row {i+1} (output generation): 'YEAR' is empty for {ghg_id}. Row: {in_row}"
                    )
                    _COUNTERS_COMPANIES["year_does_not_exist"].add(
                        (company_id, ghg_id))

                percent_own = in_row.get(CSV_PARENT_CO_PERCENT_OWN_COL,
                                         '').strip()
                if not percent_own:
                    logging.warning(
                        f"⚠️ Skipping row {i+1} (output generation): 'PARENT_CO_PERCENT_OWN' is empty for {ghg_id}. Row: {in_row}"
                    )
                ...
                if company_id.lower() not in processed_companies:
                    ...
                    if zip_code:
                        ...
                        logging.warning(
                            f"⚠️ Missing zip code for company: {company_id}, year: {year}. Row: {in_row}"
                        )
                        _COUNTERS_COMPANIES["missing_zip"].add(
                            (company_id, year))

                    county = _get_county(company_id, zip_for_county, year)

                    street_address = in_row.get(
                        CSV_PARENT_CO_STREET_ADDRESS_COL, '').strip()
                    city = in_row.get(CSV_PARENT_CO_CITY_COL, '').strip()
                    state = in_row.get(CSV_PARENT_CO_STATE_COL, '').strip()
                    zip_part_for_address = in_row.get(CSV_PARENT_CO_ZIP_COL,
                                                      '').strip()[:5]

                    full_address = ", ".join(
                        filter(
                            None,
                            [street_address, city, state, zip_part_for_address
                            ]))
    ...
    table_mcf_path = os.path.join(output_path_info,
                                  _OUT_FILE_PREFIX + "Table.mcf")
    ownership_mcf_path = os.path.join(output_path_ownership,
                                      _OUT_FILE_PREFIX + "Ownership.mcf")
    table_tmcf_path = os.path.join(output_path_info,
                                   _OUT_FILE_PREFIX + "Table.tmcf")
    ownership_tmcf_path = os.path.join(output_path_ownership,
                                       _OUT_FILE_PREFIX + "Ownership.tmcf")
...
def process_svobs(svobs_path, facility_company_ownership, facility_svo_dict):
    ...
    with open(out_path + ".csv", "w", newline='') as svofp:
        writer = csv.DictWriter(svofp, _SVOBS_CLEAN_CSV_HDR)
        writer.writeheader()
...
def main(argv):
    ...
    ownership_path_prefix = os.path.join(output_path_ownership,
                                         _OUT_FILE_PREFIX)
