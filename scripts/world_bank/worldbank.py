...
""" Fetches, cleans, outputs TMCFs and CSVs for all WorldBank development indicator codes provided in WorldBankIndicators.csv
for all years for all countries provided in WorldBankCountries.csv """
...
def group_stat_vars_by_observation_properties(indicator_codes):
  ...
  for permutation in list(itertools.product([False,True],repeat=len(properties_belonging_to_stat_var_observation))):
    ...
    tmcfs_for_stat_vars.append((base_template_mcf, cols_to_include_in_csv, list(indicator_codes.loc[codes_that_match.index]['INDICATOR_CODE'])))
...
def download_indicator_data(worldbank_countries, indicator_codes):
  ...
  Additionally, creates a mapping from isoCode to MCF node.
    ...
    Returns:
      worldbank_dataframe: Indicator data for all countries and all indicators provied.
...
def output_csv_and_tmcf_by_grouping(worldbank_dataframe, tmcfs_for_stat_vars, indicator_codes, country_mcf):
  """ Outputs TMCFs and CSVs for each grouping of stat vars. 
    ...
    Args:
      worldbank_dataframe: Dataframe containing all indicators for all countries
      tmcfs_for_stat_varss:  Array of tuples of templace MCF, columns on stat var observations, indicator codes for that template MCF
      indicator_codes: Pandas dataframe with INDICATOR_CODES to include
  ...
  for index, enum in enumerate(tmcfs_for_stat_vars):
    tmcf, stat_var_obs_cols_to_include_in_csv, stat_vars = enum
    ...
    if len(stat_vars) != 0:
      ...
      if len(stat_var_obs_cols_to_include_in_csv) > 1:
        matching_csv = pd.merge(matching_csv, indicator_codes[stat_var_obs_cols_to_include_in_csv], on="IndicatorCode")
