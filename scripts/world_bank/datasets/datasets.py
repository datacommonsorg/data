...
def write_observations_from_zip(zip_file, svs):
  ...
  if len(csv_rows) == 0:
    logging.info('SKIPPED writing obs file, no observations extracted from %s', zip_file)
    return

  obs_file_name = f"{zip_file.split('/')[-1].split('.')[0]}_obs.csv"
  obs_file_path = f"{OBSERVATIONS_DIR}/{obs_file_name}"
  logging.info('Writing %s observations from %s to %s', len(csv_rows), zip_file, obs_file_path)
    ...
    if file_names is None or len(file_names) != 2:
      ...
      with zip.open(data_file, 'r') as csv_file:
        ...
        for data_row in data_rows:
          obs_csv_rows.extend(get_observations_from_data_row(data_row, svs))
