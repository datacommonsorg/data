...
## from the filename of src csv file (which details the date-interval), reformat to YYYY or YYYY-MM
def format_date(file_path, time_interval_type):
    ...
    date_formats = {
        ...
        "agg_5year": d_str[:4]  # fname ex: 2017to2021.csv
    }
    ...
    with open(output_csv, 'w', newline='') as f_out:
        ...
        for interval_type in time_interval_types:
            for climate_var in ["ppt", "tmin", "tmax"]:
                ## file-path ex: "./data/prism/daily/county/agg_yearly/ppt/stats/2021.csv"
                path = f"{src_fldr}/{interval_type}/{climate_var}/stats"
                ## County-level statistics for each monthly/yearly/5-yearly interval 
                ##  are stored in individual csv files (i.e. 2021.csv or 202112.csv)
...
if __name__ == '__main__':
    src_fldr = f"raw_data/prism/daily/county"
    output_csv = f"WeatherVariability_Counties.csv"
    main(src_fldr, output_csv)