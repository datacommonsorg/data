...
flags.DEFINE_string('sites_file', '', \
  'Location of air quality sites file (local/gcs).') #optional
flags.DEFINE_string('output_bucket', '', \
...
def main(_):
    ...
    if data_file and data_file.startswith('gs://'):
        self.download_gcs_file(
            data_file.split('/')[2],
            data_file.split('/')[3])
        data_file = data_file.split('/')[3]
    in_df = loader.get_feed(data_file)

    logging.info('Generting sites data...')
    sites_file = FLAGS.sites_file
    if sites_file and sites_file.startswith('gs://'):
        loader.download_gcs_file(
            sites_file.split('/')[2],
            sites_file.split('/')[3])
        sites_file = sites_file.split('/')[3]
    ...
    if FLAGS.output_bucket:
        logging.info('Uploading results to GCS...')
        loader.upload_gcs_file(FLAGS.output_bucket, OUTPUT_FILE + '.csv')
        loader.upload_gcs_file(FLAGS.output_bucket, OUTPUT_FILE + '_sites.csv')
