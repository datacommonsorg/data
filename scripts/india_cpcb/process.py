  ...
  def generate_sites(self, in_df: pd.DataFrame, sites_file: str, \
    ...
    if sites_file:
      ...
      sites = pd.DataFrame(columns=[
        "SiteName","City","State","Country","DistrictDCID","LGDDistrictCode","SiteId","Coordinates"])
...
def main(_):
  ...
  if FLAGS.dataFile and FLAGS.dataFile.startswith("gs://"):
    self.download_gcs_file(FLAGS.dataFile.split("/")[2], flags.dataFile.split("/")[3])
  ...
  if FLAGS.sitesFile and FLAGS.sitesFile.startswith("gs://"):
    loader.download_gcs_file(FLAGS.sitesFile.split("/")[2], FLAGS.sitesFile.split("/")[3])
