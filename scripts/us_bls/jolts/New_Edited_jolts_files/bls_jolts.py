...
def generate_cleaned_dataframe():
  ...
  """
    ...
    # static_url = https://download.bls.gov/pub/time.series/jt/jt.data.
    #series_desc = pd.read_csv("https://download.bls.gov/pub/time.series/jt/jt.series", storage_options = {'User-Agent': 'Mozilla/5.0'})
    ...
    #old
    # series_desc = pd.read_csv("https://download.bls.gov/pub/time.series/jt/jt.series",converters={'industry_code': str}, sep="\\s+")
    ...
    for schema_name, population_type, job_change_event, df in schema_mapping:
     ...
     #   jolts_df = pd.concat(jolts_df,df) # need to change append to concat
 #       jolts_df = jolts_df._append(df)
    ...
    series_cols = ['industry_code', 'state_code', 'seasonal', 'ratelevel_code']
