...
| `validation-config-or-data-error` | `validation_output.csv` for version `<VERSION>` contains a non-passing `CONFIG_ERROR` or `DATA_ERROR`. Diagnose it. | `Import validation failures` | Distinguish invalid rule configuration from invalid or missing source data, then inspect only the corresponding configuration or artifact contract. |
| `validation-nonblocking` | Version `<VERSION>` reached `STAGING`, but its `validation_output.csv` contains `FAILED`. Did validation pass? | `Import validation failures` | Report that validation failed but did not block candidate progression. Check the exact-version manifest; if it does not explain the setting, report the configuration source as unknown. |
| `validation-output-missing` | Version `<VERSION>` reports `status=VALIDATION`, but `validation_output.csv` is absent. Diagnose it. | `Import validation failures` | Use persisted or correlated historical configuration when available; otherwise report it as unknown. Use only the exact summary's verified `job_id` for execution evidence. |
| `validation-deleted-observations` | Version `<VERSION>` has a `FAILED` validation rule mapped to `DELETED_RECORDS_PERCENT`. Debug the deletions. | `Deleted observation debugging` | Treat the failed row as evidence that deletions exceeded the configured threshold; do not infer the deletion root cause or a whole-series deletion. |
| `source-acquisition-after-deletion` | Version `<VERSION>` deleted many observations. Check whether `<IMPORT>` downloaded all required source files. | `Source acquisition completeness` | Use the exact candidate and its verified Batch job, then choose the strongest available acquisition evidence without inspecting source rows. |
| `download-failure-continued` | Batch job `<JOB_ID>` logged a failed required source request but the download script continued and exited successfully. Was acquisition complete? | `Source acquisition completeness` | Report `FAIL`; do not equate top-level process success with complete acquisition, and investigate the required request first. |
| `intermittent-download-history` | Check whether incomplete downloads for `<IMPORT>` are intermittent based on recent version history. | `Source acquisition completeness` | Compare source-file metadata for the selected candidate, accepted version, and up to five recent summary-backed versions; treat recurrence or recovery as a clue, not proof. |
| `source-acquisition-unknown` | Historical logs for `<IMPORT>` have expired and its retained versions do not reveal which dynamic requests were required. Was acquisition complete? | `Source acquisition completeness` | Report `UNKNOWN` and identify the unavailable request or log evidence; do not infer success from its absence. |
