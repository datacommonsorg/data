...
FLAGS = flags.FLAGS
flags.DEFINE_string('data_type', "sat_act", 'The type of data to download: "sat_act".')
flags.mark_flag_as_required('data_type')

BASE_URL_TEMPLATE = "https://civilrightsdata.ed.gov/assets/ocr/docs/{}.zip"

DATA_CONFIGS = {
    # SAT/ACT Data Keywords
    "sat_act": {
        "keywords": [r'SAT.*ACT'],
        "search_constraint": {
            "2011-12": "Pt 2-27b - Stud who took SAT or ACT.xlsx",
        },
        "output_name_fragment": "SAT_ACT_Participation",
    }
}
    ...
    try:
        if file_path.suffix == '.xlsx':
            ...
            try:
                ...
                logging.warning(f"Warning: No data found in {file_path.name}. Skipping file.")
                return
        else:
