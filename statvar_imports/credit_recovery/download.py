        ...
        for filename in filenames:
            for keyword, folder_name in KEYWORD_TO_FOLDER_MAP.items():
                if keyword.lower() in filename.lower():
                    ...
                    break
    logging.info(f"Successfully moved {moved_count} files for year {year}.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            ...
            if not download_success:
                ...
                logging.info(f"Successfully processed {os.path.basename(url)}")
...
def main():
    ...
    logging.info(f"Ensured base directory exists: {FILES_DIRECTORY}")
