...
def melt_year_and_flag_columns(input_csv_file, output_csv_file):
    ...
    try:
        ...
        df1.drop(columns=[
            ...
            if (col.startswith('Y') and not col.endswith('F'))
        ])
        ...
        if 'Months' in melted_df.columns:
            melted_df['Months'] = melted_df['Months'].replace(
                'Annual value', 'Annual')
        ...
        if 'Area' in melted_df.columns:
            melted_df['Area'] = melted_df['Area'].str.replace(',',
                                                              '',
                                                              regex=False)
        ...
        logging.info(
            f"Successfully melted year and flag columns from '{input_csv_file}' to '{output_csv_file}'"
        )
