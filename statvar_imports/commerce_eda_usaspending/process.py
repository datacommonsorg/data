...
STATE_ABBREV = {
    ...
    "WI": "Wisconsin", "WY": "Wyoming", "VI": "U.S. Virgin Islands"
}
...
def fetch_usaspending_data(start_year, end_year, session=None):
    ...
    while True:
        payload = {
            "filters": {
                "agencies": [{"type": "awarding", "tier": "subtier", "name": "Economic Development Administration"}],
...
def process_data(awards, start_year, end_year, output_path):
    ...
    # Aggregate
    agg_df = df.groupby(["Place", "Category", "Year"])["Amount"].sum().reset_index()
    ...
    final_df["place_idx"] = final_df["Place"].apply(lambda x: places_sorted.index(x))
    category_order = ["Total", "Economic Adjustment Assistance", "Planning", "Public Works", "Regional Innovation Strategies", "Technical Assistance"]
    def get_cat_idx(cat):
        if cat in category_order:
            return category_order.index(cat)
        return 100
    final_df["cat_idx"] = final_df["Category"].apply(get_cat_idx)
    final_df = final_df.sort_values(by=["place_idx", "cat_idx", "Year"]).reset_index(drop=True)
    final_df = final_df.drop(columns=["place_idx", "cat_idx"])
    
    # Format amount
    final_df["Amount"] = final_df["Amount"].apply(lambda val: str(int(val)) if val > 0 else "")
    # Drop rows with empty amounts
    final_df = final_df[final_df["Amount"] != ""]
    
    final_df = final_df.rename(columns={"Category": "State or Territory / EDA Program", "Amount": "Value"})
    # Reorder columns
    final_df = final_df[["Place", "State or Territory / EDA Program", "Year", "Value"]]
...
def main(argv):
    ...
    output_path = "output/Investment_cleaned.csv"
