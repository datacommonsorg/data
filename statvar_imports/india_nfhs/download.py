...
# --- FIX for import error based on provided paths ---
...
def load_api_keys(filepath):
    """Loads API keys from a JSON file and returns them as a dictionary."""
    try:
        with open(filepath, 'r') as f:
            return json.load(f)
    except FileNotFoundError:
        logging.fatal(f"API keys file not found: {filepath}. Please create this file with your keys.")
        sys.exit(1)
    except json.JSONDecodeError as json_err:
        logging.fatal(f"Error decoding JSON from {filepath}: {json_err}. Please check the file's format.")
        sys.exit(1)
    except Exception as e:
        logging.fatal(f"An unexpected error occurred while loading API keys: {e}")
        sys.exit(1)
...
def main(argv):
    ...
    api_keys = load_api_keys(os.path.join(script_dir, _API_KEYS_FILE))
