...
flags.DEFINE_string("input_dir", "noaa_storm_data", "Directory containing NOAA storm data.")
...
def process_file(input_file):
    """Processes a single data file and writes its output to a temporary CSV file."""
    file_name = os.path.basename(input_file)
    logging.info(f"Starting processing for file: {file_name}")
    
    processed_rows = set()
    
    temp_output_file = os.path.join(FLAGS.temp_dir, f"processed_{file_name}")
            ...
            for row in reader:
                try:
                    ...
                    if not valid_lat(begin_lat) or not valid_lng(begin_lon):
                        logging.warning(f"Invalid coordinates for row in {file_name}: lat={begin_lat}, lon={begin_lon}")
                        continue
                    ...
                    output_row = [
                        storm_event, storm_episode, storm_episode_name, date, event_type,
                    ...
                    logging.error(f"Error processing row in {file_name}: {row}\n{str(e)}")
    
    logging.info(f"Finished processing for file: {file_name}")
...
def main(argv):
    ...
    input_files = glob.glob(os.path.join(FLAGS.input_dir, "StormEvents_details*.csv"))
    if not input_files:
        logging.error(f"No data files found in {FLAGS.input_dir}.")
        raise RuntimeError(f"No data files found in {FLAGS.input_dir}.")

    # Use multiprocessing to process files in parallel
    num_processes = multiprocessing.cpu_count()
    logging.info(f"Starting parallel processing with {num_processes} workers for {len(input_files)} files.")
    
    with multiprocessing.Pool(processes=num_processes) as pool:
        temp_files = pool.map(process_file, input_files)

    # Combine results from temporary files
    logging.info("Combining results from all processed files.")
    header = [
        "stormEvent", "stormEpisode", "stormEpisodeName", "observationDate", "eventType", "directInjuries",
