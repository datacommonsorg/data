...
class BrazilDownloadScriptTest(unittest.TestCase):
    ...
    def test_get_robust_session_configuration(self):
        ...
        self.assertTrue(
            expected_status_codes.issubset(
                set(adapter.max_retries.status_forcelist)))
    ...
    def test_get_available_periods_success(self, mock_get):
        ...
        mock_response.json.return_value = [
            {
                "id": "202103",
                "literals": ["3º trimestre 2021"]
            },
            {
                "id": "202104",
                "literals": ["4º trimestre 2021"]
            },
            {
                "id": "202201",
        ...
        mock_response.json.return_value = [{
            ...
            "resultados": [{
                "classificacoes": [{
                    "id": "629",
                    "categoria": {
                        "32385": "Pessoas de 14 anos ou mais"
                    }
                }],
                "series": [{
                    "serie": {
    ...
    def test_fetch_panel_data_all_panels(self, mock_fetch):
        """Tests data fetching, matrix construction, and CSV writing for panels 1 through 4."""
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            with patch.object(brazil_download_script, "DOWNLOAD_DIR", tmp_dir):
                for panel_idx in range(1, 5):
                    brazil_download_script.fetch_panel_data(
                        "Brasil", "N1[1]", panel_idx, periods)
                    folder_name = brazil_download_script.PANEL_FOLDER_MAP[
                        panel_idx]
                    ...
                    self.assertEqual(
                        df.iloc[0, 0],
                        "Pesquisa Nacional por Amostra de Domicílios Contínua - Divulgação Trimestral"
                    )
    ...
    def test_fetch_panel_data_invalid_index(self):
        ...
        with self.assertRaises(ValueError):
            brazil_download_script.fetch_panel_data("Brasil", "N1[1]", 99,
                                                    periods)
    ...
    def test_main_orchestration(self, mock_get_periods, mock_fetch_panel,
                                mock_sleep):
    ...
    def test_test_data_csv_fixtures_structure(self):
        ...
        test_data_dir = os.path.join(_SCRIPT_DIR, "test_data")
        expected_fixtures = [
            "average_real_income_input.csv",
            "employment_and_unemployment_labor_force_input.csv",
            "mass_income_input.csv",
            "population_economic_sector_input.csv",
        ]
        # Concise .csv fixtures must exist, be < 100 KB, and have valid SIDRA structure
        for fixture_file in expected_fixtures:
            path = os.path.join(test_data_dir, fixture_file)
            self.assertTrue(os.path.exists(path),
                            f"CSV fixture missing: {path}")
            size_kb = os.path.getsize(path) / 1024
            self.assertLess(
                size_kb, 100,
                f"Fixture {fixture_file} exceeds 100 KB: {size_kb:.2f} KB")
            ...
            self.assertEqual(df.iloc[3, 0], "Indicador")
            self.assertTrue(
                any("trimestre" in str(col).lower()
                    for col in df.iloc[4].values))
