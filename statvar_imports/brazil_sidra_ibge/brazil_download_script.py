...
def wait_for_downloads(initial_file_count, total_expected_files, timeout=60):
    """
    Waits for all new download files to appear in the directory.
    """
    start_time = time.time()
    while True:
        current_files = glob.glob(os.path.join(DOWNLOAD_DIR, '*.xlsx'))
        
        # Check if the number of new files matches the number of expected downloads
        if len(current_files) == (initial_file_count + total_expected_files):
            # Also ensure no temporary files (.crdownload) are present
            if not any(f.endswith(".crdownload") for f in os.listdir(DOWNLOAD_DIR)):
                logging.info(f"All {total_expected_files} downloads completed successfully.")
                return True
...
def rename_and_move_downloaded_file(place_name, panel_index, original_filename):
    """Renames the downloaded file and moves it to the correct subfolder."""
    try:
        ...
        new_filename = f"{place_name.replace(' ', '_')}_Panel_{panel_index}_{original_filename}"
        new_filepath = os.path.join(destination_dir, new_filename)
        
        downloaded_filepath = os.path.join(DOWNLOAD_DIR, original_filename)
        
        if not os.path.exists(downloaded_filepath):
            logging.warning(f"File '{original_filename}' not found for {place_name} (Panel {panel_index}).")
            return
            
        os.rename(downloaded_filepath, new_filepath)
        logging.info(f"Moved and renamed file '{original_filename}' to '{new_filepath}'")
...
def download_panel(driver, panel_index, place_name):
    """Locates a panel and triggers the download."""
    try:
...
def main(argv):
    ...
    try:
        ...
        # Initial file count before any downloads begin
        initial_file_count = len(glob.glob(os.path.join(DOWNLOAD_DIR, '*.xlsx')))

        # Download all panels for 'Brasil'
        logging.info("\n▶ Processing: Brasil")
        try:
            for panel_index in range(1, 5):
                download_panel(driver, panel_index, 'Brasil')
            
            wait_for_downloads(initial_file_count, 4)
            
            # Get the new files after the downloads are complete
            new_files = sorted(glob.glob(os.path.join(DOWNLOAD_DIR, '*.xlsx')), key=os.path.getctime)[-4:]
            
            # Rename and move the new files
            for i, panel_index in enumerate(range(1, 5)):
                rename_and_move_downloaded_file('Brasil', panel_index, os.path.basename(new_files[i]))
        ...
        for place_name in options_text:
            if not place_name or place_name == 'Brasil':
                continue
            ...
            try:
                ...
                # Initial file count before starting downloads for this place
                initial_file_count_place = len(glob.glob(os.path.join(DOWNLOAD_DIR, '*.xlsx')))
                for panel_index in range(1, 5):
                    download_panel(driver, panel_index, place_name)
                wait_for_downloads(initial_file_count_place, 4)
                ...
                # Rename and move the new files.
                for i, panel_index in enumerate(range(1, 5)):
                    rename_and_move_downloaded_file(place_name, panel_index, os.path.basename(new_files[i]))
                ...
                logging.fatal(f"  ✖ Critical failure selecting or downloading for {place_name}: {e}. Exiting script.")
