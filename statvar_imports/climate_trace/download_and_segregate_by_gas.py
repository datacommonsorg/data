...
def download_and_process_zip(url, country_iso, gas):
  ...
  try:
    ...
    if e.response is not None and e.response.status_code == 404:
      logging.warning(
          f"    -> Not found (404) for {country_iso} ({gas}) at {url}")
    else:
      logging.error(
          f"    -> HTTP Error for {country_iso} ({gas}) at {url}: {e}")
    return None
  except Exception as e:
    logging.error(
        f"    -> Unexpected error for {country_iso} ({gas}) at {url}: {e}")
    return None
...
def download_and_segregate_by_gas():
  ...
  try:
    ...
    logging.info(
        f"Successfully fetched {len(api_country_codes)} countries from API.")
  except requests.exceptions.RequestException as e:
    logging.error(f"Error: Could not fetch country list from API: {e}")

  local_country_codes = set()
  check_country_path = os.path.join(_SCRIPT_DIR, 'check_country.csv')
  try:
    with open(check_country_path, 'r') as f:
      local_country_codes = {line.strip() for line in f if line.strip()}
    logging.info(
        f"Read {len(local_country_codes)} countries from 'check_country.csv'.")
  except FileNotFoundError:
    logging.warning(
        "Warning: 'check_country.csv' not found. Will only use countries from"
        " API.")

  combined_codes = sorted(list(api_country_codes.union(local_country_codes)))
  if not combined_codes:
    raise RuntimeError(
        "No country codes found from API or check_country.csv to process.")
  ...
  output_dir = os.path.join(_SCRIPT_DIR, "input_files")
  os.makedirs(output_dir, exist_ok=True)

  all_gases = sorted(
      list(set(gas for gases in country_urls.values() for gas in gases)))
  logging.info(
      f"Found data for the following gas types: {', '.join(all_gases)}\n")
  ...
  for gas in all_gases:
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
      future_to_item = {
          executor.submit(
              download_and_process_zip, item['url'], item['iso'], gas): item
    ...
    if not gas_dataframes:
      logging.info(
          f"No data was downloaded for {gas}. The output file will not be"
          " created.")
      continue
    ...
    try:
      ...
      if 'emissions_quantity' in final_df.columns:
        ...
        final_df['emissions_quantity'] = final_df['emissions_quantity'].apply(
            lambda x: format(x, '.16f').rstrip('0').rstrip('.')
      ...
      created_files += 1
      logging.info(
          f"  -> Successfully created {output_filename} with {len(final_df)}"
          " rows.\n")
    except Exception as e:
      logging.error(
          f"  -> An error occurred during the final processing for {gas}:"
          f" {e}\n")
      raise
  ...
  if failed_downloads:
    logging.warning(
        f"The following {len(failed_downloads)} downloads failed or returned"
        " no data:")
  ...
  if created_files == 0:
    raise RuntimeError("No output files were generated across any gas types.")
