...
if [ -f "$INPUT_5" ] && grep -q '^"[^"]*","  Ulaanbaatar"' "$INPUT_5"; then
    ...
    TMP_FILE="$INPUT_5.tmp.$$"
    trap 'rm -f "$TMP_FILE"' EXIT
    if sed '/^"[^"]*","  Ulaanbaatar"/d' "$INPUT_5" > "$TMP_FILE" && [ -s "$TMP_FILE" ]; then
        mv "$TMP_FILE" "$INPUT_5"
    fi
    trap - EXIT
fi
