...
def download_who_tb_data_with_iso3():
    ...
    # Check for any countries that didn't get an iso3 code
    unmatched_countries = merged_df[merged_df['iso3'].isnull()]['COUNTRY'].unique()
    if unmatched_countries.size > 0:
        print(f"Warning: {len(unmatched_countries)} countries could not be matched to an iso3 code and will be dropped:")
