...
"""Preprocesses Zurich BEV390OD3903 dataset to generate all marginal and total roll-up slices."""
...
DEFAULT_INPUT_RAW = os.path.join(CURRENT_DIR, 'input_files', 'BEV390OD3903.csv')
...
REQUIRED_COLS = [
    ...
    'HerkunftLang', 'AnzBestWir'
]
OUTPUT_COLS = [
    'StichtagDatJahr', 'QuarLang', 'AlterV10Kurz', 'SexKurz', 'HerkunftLang',
    'AnzBestWir'
]


def process_rollups(df: pd.DataFrame) -> pd.DataFrame:
    """Generates 3-way, 2-way, 1-way, and total population rollups across geographic levels.

  Args:
    df: Input pandas DataFrame containing raw Zurich BEV3903 population data.
  ...
  """
    ...
    for _, col in spatial_levels:
        ...
        group_keys = ['StichtagDatJahr', 'QuarLang']

        # 1. Full 3-way breakdown (Age + Gender + Nativity)
        df_full = sub_df.groupby(group_keys +
                                 ['AlterV10Kurz', 'SexKurz', 'HerkunftLang'],
                                 as_index=False)[val_col].sum(min_count=1)

        # 2. 2-way marginal breakdowns
        df_age_sex = sub_df.groupby(group_keys + ['AlterV10Kurz', 'SexKurz'],
                                    as_index=False)[val_col].sum(min_count=1)
        df_age_sex['HerkunftLang'] = ''

        df_age_hel = sub_df.groupby(group_keys +
                                    ['AlterV10Kurz', 'HerkunftLang'],
                                    as_index=False)[val_col].sum(min_count=1)
        df_age_hel['SexKurz'] = ''

        df_sex_hel = sub_df.groupby(group_keys + ['SexKurz', 'HerkunftLang'],
                                    as_index=False)[val_col].sum(min_count=1)
        df_sex_hel['AlterV10Kurz'] = ''

        # 3. 1-way marginal breakdowns (Age only, Gender only, Nativity only)
        df_age = sub_df.groupby(group_keys + ['AlterV10Kurz'],
                                as_index=False)[val_col].sum(min_count=1)
        df_age['SexKurz'] = ''
        df_age['HerkunftLang'] = ''

        df_sex = sub_df.groupby(group_keys + ['SexKurz'],
                                as_index=False)[val_col].sum(min_count=1)
        df_sex['AlterV10Kurz'] = ''
        df_sex['HerkunftLang'] = ''
        ...
        for slice_df in [
                df_full, df_age_sex, df_age_hel, df_sex_hel, df_age, df_sex,
                df_hel, df_tot
        ]:
