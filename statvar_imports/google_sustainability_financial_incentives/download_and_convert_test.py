...
class DownloadAndConvertTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_json_path = os.path.join(
            self.test_data_dir, 'financial_incentives_prod_data.json')
    ...
    def test_find_latest_dated_folder(self):
        ...
        with patch('download_and_convert.storage.Client') as mock_client:
            ...
            result = download_and_convert.find_latest_dated_folder(
                'test_bucket', 'source_csv/sustainability_financial_incentives')

            self.assertEqual(result, '2025_07_01')
            mock_client.assert_called_once()
            mock_bucket.list_blobs.assert_called_once_with(
                prefix='source_csv/sustainability_financial_incentives/')
    ...
    def test_download_json_file(self):
        ...
        with patch('download_and_convert.storage.Client') as mock_client:
            ...
            result = download_and_convert.download_json_file(
                'test_bucket', 'test/path/file.json', local_path)
    ...
    def test_convert_json_to_csv(self):
        ...
        expected_csv_path = os.path.join(
            self.test_data_dir, 'expected_financial_incentives_prod_data.csv')

        result = download_and_convert.convert_json_to_csv(
            self.test_json_path, self.test_csv_path)
