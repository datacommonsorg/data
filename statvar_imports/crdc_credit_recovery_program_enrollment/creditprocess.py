...
def process_credit_files():
    ...
    logging.info(f"--- Processing files in {credit_dir} ---")
    ...
    for filename in os.listdir(credit_dir):
        ...
        try:
            ...
            if not year_match:
                logging.warning(f"Skipping '{filename}': Year not found in filename.")
                continue
            ...
            if 'SCH_CREDITRECOVERYENR' not in df.columns:
                logging.warning(f"Skipping '{filename}': 'SCH_CREDITRECOVERYENR' column not found.")
                continue
