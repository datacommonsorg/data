...
def preprocess_file(filepath: str, year: int):
    """Adds 'ncesid' and 'year' columns to the given CSV or XLSX file."""
    try:
        ...
        if filepath.endswith('.csv'):
            ...
            logging.warning(
                f"Unsupported file type for preprocessing: {filepath}")
            return

        # Create 'ncesid'
        if "LEAID" in df.columns and "SCHID" in df.columns:
            df["LEAID"] = df["LEAID"].str.zfill(7)
            df["SCHID"] = df["SCHID"].str.zfill(5)
            ncesid_series = df["LEAID"] + df["SCHID"]
            df.insert(0, "ncesid", ncesid_series)
            df.insert(1, "YEAR", year)
            logging.info(
                f"Added 'ncesid' and 'YEAR' columns to {os.path.basename(filepath)}"
            )
        else:
            logging.warning(
                f"Could not find 'LEAID' and/or 'SCHID' columns in {os.path.basename(filepath)}. Adding only 'YEAR' column."
            )
        ...
        logging.fatal(f"Could not preprocess {filepath}: {e}")
        ...
        for item_path in glob.glob(search_pattern, recursive=True):
            ...
            if (extension in ['.csv', '.xlsx'] and
                    (category_name.lower() in base.lower() or
                     (year == 2015 and "CRDC 2015-16 School Data" in base and "layout" not in base.lower()))):
                if 'lea' in base.lower() and extension == '.xlsx':
                    logging.info(
                        f"Skipping and removing Excel file: '{filename}' because it contains 'LEA'."
                    )
                    os.remove(item_path)
                    continue
                ...
                # Check for 'enrollment' or 'algebra_ii' in the filename and remove if found
                if "enrollment" in clean_base or "algebra_ii" in clean_base:
                    logging.info(f"Skipping and removing file: '{new_filename}' because it contains 'enrollment' or 'algebra_ii'.")
