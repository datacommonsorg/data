...
sys.path.append(os.path.join(_SCRIPT_DIR, '..', '..', 'tools', 'statvar_importer'))
...
def find_latest_dated_folder(bucket_name: str, base_path: str) -> Optional[str]:
    ...
    try:
        ...
        if not dated_folders:
            logging.error(f"No dated folders found in {bucket_name}/{base_path}")
            return None
...
def download_json_file(bucket_name: str, gcs_path: str, local_path: str) -> bool:
    ...
    try:
        ...
        logging.error(f"Error downloading file from {gcs_path} to {os.path.abspath(local_path)}: {e}")
        return False
...
def convert_json_to_csv(json_path: str, csv_path: str) -> bool:
    ...
    """
    try:
        logging.info(f"Starting JSON to CSV conversion from {os.path.abspath(json_path)}")
        ...
        if 'incentiveSummaries' not in data:
            logging.error(f"Key 'incentiveSummaries' not found in JSON file: {os.path.abspath(json_path)}")
            return False
        
        incentive_summaries = data['incentiveSummaries']
        
        if not isinstance(incentive_summaries, list):
            logging.error(f"'incentiveSummaries' is not a list but {type(incentive_summaries).__name__} in: {os.path.abspath(json_path)}")
            return False
        
        if len(incentive_summaries) == 0:
            logging.error(f"'incentiveSummaries' list is empty in: {os.path.abspath(json_path)}")
            return False
        
        logging.info(f"Found {len(incentive_summaries)} incentive summaries in JSON file")
        ...
        logging.info(f"Creating temporary JSON file: {os.path.abspath(temp_json_path)}")
        
        # Write the incentiveSummaries array to temporary file
        with open(temp_json_path, 'w', encoding='utf-8') as f:
            json.dump(incentive_summaries, f, indent=2, ensure_ascii=False)
        
        logging.info(f"Successfully created temporary JSON file with {len(incentive_summaries)} records")
        
        # Use existing function to convert temporary JSON to CSV
        file_json_to_csv(temp_json_path, csv_path, set_key_column=False)
        
        logging.info(f"Successfully converted {os.path.abspath(temp_json_path)} to {os.path.abspath(csv_path)}")
        logging.info(f"Temporary JSON file preserved at: {os.path.abspath(temp_json_path)}")
        
        return True
            
    except Exception as e:
        logging.error(f"Error converting JSON to CSV from {os.path.abspath(json_path)} to {os.path.abspath(csv_path)}: {e}")
        return False
...
def main(argv):
    ...
    latest_folder = find_latest_dated_folder(FLAGS.gcs_bucket, FLAGS.gcs_base_path)
