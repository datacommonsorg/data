...
def get_authorized_session(
        project_id: str = DEFAULT_PROJECT_ID) -> AuthorizedSession:
    ...
    try:
        ...
        logging.info(
            "ADC refresh failed (%s); using gcloud auth print-access-token...",
            adc_err)
...
def resolve_composer_webserver_url(
    ...
    env_url = (f"https://composer.googleapis.com/v1/projects/{project_id}"
    ...
    try:
        ...
        airflow_uri = subprocess.check_output(cmd, text=True,
                                              timeout=30).strip()
        if airflow_uri:
            return airflow_uri.rstrip("/")
        raise RuntimeError(
            f"Could not resolve Composer webserver URI via REST or CLI: {rest_err}"
        ) from rest_err

    raise RuntimeError(
        f"Could not find config.airflowUri in Composer environment response: {data}"
    )
...
def check_dag_import_errors(session: AuthorizedSession,
    ...
    if import_errors:
        logging.error("Airflow reported %d DAG import error(s):",
                      len(import_errors))
        ...
        raise RuntimeError(
            f"Aborting E2E test due to {len(import_errors)} DAG import error(s) in Airflow."
        )
    ...
    while time.time() - start_time < max_wait_sec:
        ...
        if resp.status_code == 200:
            ...
            if dag_info.get("is_paused"):
                logging.info(
                    "DAG '%s' is currently paused. Unpausing for E2E test...",
                    dag_id)
                patch_resp = session.patch(dag_url,
                                           json={"is_paused": False},
                                           timeout=30)
                patch_resp.raise_for_status()
                dag_info = patch_resp.json()
            logging.info("DAG '%s' is active and ready (is_paused=%s).", dag_id,
                         dag_info.get("is_paused"))
            return dag_info
        logging.info(
            "Waiting for DAG '%s' to be registered in Airflow (status %d)...",
            dag_id, resp.status_code)
        time.sleep(10)

    raise TimeoutError(
        f"DAG '{dag_id}' was not found in Airflow after {max_wait_sec}s.")
    ...
    for ti in task_instances:
        ...
        if state in ("failed", "upstream_failed") and t_id:
            ...
            try:
                ...
                logging.warning(
                    "Could not retrieve log for failed task '%s': %s", t_id, ex)
...
def run_e2e_test(
    ...
    if sync_wait_sec > 0:
        logging.info(
            "Waiting %ds for Composer DAG processor to sync latest GCS files...",
            sync_wait_sec)
    ...
    try:
        while time.time() - start_time < timeout_sec:
            ...
            task_summary = ", ".join(
                f"{ti.get('task_id')}:{ti.get('state') or 'none'}" for ti in
                sorted(task_instances, key=lambda x: x.get("task_id", "")))
            elapsed = int(time.time() - start_time)
            logging.info("[%04ds] DAG State: %-10s | Tasks: %s", elapsed,
                         dag_state.upper(), task_summary)

            # Check if HumanApprovalSensor is paused waiting for approval
            for ti in task_instances:
                if ti.get("task_id") == "await_human_approval" and ti.get(
            ...
            raise TimeoutError(
                f"DAG run '{run_id}' timed out after {timeout_sec}s (last state: {dag_state})."
            )

    finally:
        if approval_variable_set:
            logging.info(
                "Cleaning up temporary Airflow Variable 'PROD_APPROVE_ALL'...")
            try:
                session.delete(f"{var_url}/PROD_APPROVE_ALL", timeout=30)
            except Exception as ex:
                logging.warning(
                    "Failed to delete temporary variable PROD_APPROVE_ALL: %s",
                    ex)

    # 5. Evaluate final state & fetch XCom summary
    if dag_state != "success":
        logging.error("E2E DAG run '%s' FAILED with state '%s'!", run_id,
                      dag_state)
        dump_failed_task_logs(session, webserver_url, dag_id, run_id,
                              task_instances)
        raise RuntimeError(
            f"E2E test failed: DAG '{dag_id}' run '{run_id}' finished with state '{dag_state}'."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run E2E test for an Airflow DAG in Cloud Composer.")
    parser.add_argument("--project-id",
                        default=os.environ.get("PROJECT_ID",
                                               DEFAULT_PROJECT_ID))
    parser.add_argument("--location",
                        default=os.environ.get("LOCATION", DEFAULT_LOCATION))
    parser.add_argument("--composer-env",
                        default=os.environ.get("COMPOSER_ENV_NAME",
                                               DEFAULT_COMPOSER_ENV))
    ...
    parser.add_argument(
        "--skip-import-job",
        action="store_true",
        default=os.environ.get("E2E_SKIP_IMPORT_JOB",
                               "").lower() in ("true", "1", "yes"),
        help=
        "Skip the Cloud Batch import job step to test orchestration quickly.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--run-golden-tests",
        action="store_true",
        default=os.environ.get("E2E_RUN_GOLDEN_TESTS", "").lower()
        in ("true", "1", "yes"),
