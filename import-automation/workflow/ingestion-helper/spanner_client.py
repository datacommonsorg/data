...
class SpannerClient:
    ...
    def update_import_status(self, import_name: str, status: str,
                             job_id: str, duration: int, schedule: str) -> bool:
        ...
        try:
            nextRefreshDate = croniter(schedule, datetime.now(timezone.utc)).get_next(datetime).date().isoformat()
            logging.info(f"Next refresh date: {nextRefreshDate} {duration}")
        except Exception as e:
            logging.error(f"Error calculating next date: {e}")

        def _record(transaction: Transaction) -> bool:
            columns = ["ImportName", "State", "JobId", "ExecutionTime", "NextRefreshDate", "StatusUpdateTimestamp"]

            values = [[import_name, status, job_id, duration, nextRefreshDate, spanner.COMMIT_TIMESTAMP]]
