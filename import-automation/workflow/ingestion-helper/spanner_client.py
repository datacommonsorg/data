...
class SpannerClient:
    ...
    def update_ingestion_history(self, workflow_id: str, job_id: str,
        ...
        def _insert(transaction: Transaction):
            ...
            values = [[
                spanner.COMMIT_TIMESTAMP, workflow_id, job_id, ingested_imports,
                metrics['execution_time'], metrics['node_count'],
                metrics['edge_count'], metrics['obs_count']
            ]]
    ...
    def has_pending_imports(self) -> bool:
        """Checks if there are any imports in PENDING state."""
        try:
            with self.database.snapshot() as snapshot:
