...
def ingestion_helper(request):
    ...
    if actionType == 'get_import_info':
        ...
        if status == 'SUCCESS':
            ...
            if not spanner.has_pending_imports():
                metrics = import_utils.get_ingestion_metrics(
                    FLAGS.project_id, FLAGS.location, job_id)
                spanner.update_ingestion_history(workflow_id, job_id,
                                                 ingested_imports, metrics)
            else:
                logging.info(
                    f"Skipping IngestionHistory update for workflow {workflow_id}"
                )
        return ('OK', 200)
