...
def run_import_job(import_name: str = "", **context) -> dict[str, Any]:
    ...
    job_spec = _build_batch_job_spec(
        cfg["imageUri"], cfg["importName"], cfg["batchImportConfig"],
...
def _run_cloud_run_job(
    ...
    logging.info("Triggering Cloud Run job '%s' via %s with args: %s", job_name, run_url, args)
    resp = session.post(run_url, json=payload, timeout=60)
    resp.raise_for_status()
    operation = resp.json()
    op_name = operation.get("name")
    if not op_name:
        raise RuntimeError(f"Cloud Run jobs.run did not return an operation name: {operation}")

    logging.info("Waiting for Cloud Run job operation '%s' to complete (timeout: %ss)...", op_name, timeout_seconds)
    ...
    while True:
        if operation.get("done"):
            if "error" in operation:
                err = operation["error"]
                raise RuntimeError(f"Cloud Run job '{job_name}' failed with error: {err}")
        ...
        if time.time() - start_time > timeout_seconds:
            raise TimeoutError(f"Cloud Run job '{job_name}' operation '{op_name}' timed out after {timeout_seconds}s.")
...
def run_validation_job(import_name: str = "", **context) -> dict[str, Any]:
    ...
    if cfg["skipImportJob"] or cfg.get("skipValidationJob"):
        ...
        return {"status": "SKIPPED", "message": "Validation job skipped by configuration"}
...
def resolve_workflow_context(context: dict[str, Any], default_import_name: str = "") -> dict[str, Any]:
    ...
    validation_job_name = cfg_val("validationJobName", f"import-validator-job{env_suffix}", "VALIDATION_JOB_NAME")
...
def ingest_prod(**context) -> dict[str, Any]:
    ...
    for gate_task_id in ("verify_golden_tests", "verify_schema_golden_gate", "pre_prod_gate"):
        gate_res = ti.xcom_pull(task_ids=gate_task_id) if ti else None
        if isinstance(gate_res, dict) and gate_res.get("status") in ("FAILURE", "FAILED"):
            logging.error("Pre-prod gate '%s' failed: %s. Blocking production promotion.", gate_task_id, gate_res)
            return {"status": "BLOCKED", "message": f"Blocked by pre-prod gate {gate_task_id}: {gate_res.get('message', 'FAILURE')}"}
...
def workflow_summary(**context) -> dict[str, Any]:
    ...
    dag = context.get("dag")
    dag_task_ids = set(dag.task_ids) if dag and hasattr(dag, "task_ids") else None
    has_golden = (
        ("verify_golden_tests" in dag_task_ids)
        if dag_task_ids is not None
        else is_golden_test_import(cfg["importName"])
    )
    stages = [
    ...
    for gate_task_id in ("verify_schema_golden_gate", "pre_prod_gate"):
        ...
        if gate_val is not None:
            results[gate_task_id] = gate_val if isinstance(gate_val, dict) else {"status": "SUCCESS", "result": gate_val}
...
base_dag_params = {
    ...
    "skipValidationJob": Param(default=False, type="boolean", description="Skip Cloud Run validation job"),
    "validationJobName": Param(default="", type="string", description="Cloud Run validation job name override"),
...
golden_dag_params = {
    "runGoldenTests": Param(default=False, type="boolean", description="Force staging golden verification gate"),
    "skipGoldenTests": Param(default=False, type="boolean", description="Skip staging golden verification gate"),
    "goldenTestTriggerId": Param(default="", type="string", description="Cloud Build trigger ID override"),
    "goldenTestBranch": Param(default="master", type="string", description="Cloud Build branch override"),
    "goldenTestProjectId": Param(default="datcom-ci", type="string", description="Cloud Build project ID override"),
    "goldenDiffBucket": Param(default="datcom-ci-test", type="string", description="GCS bucket containing golden diff summaries"),
    "autoApproveGoldenDiff": Param(default=False, type="boolean", description="Auto-approve staging golden diffs without waiting for human input"),
}
...
def build_dag(
    ...
    with dag_instance:
        ...
        pipeline: list[Any] = [batch_task, validation_task, staging_task, staging_wait]
        if has_golden_check:
