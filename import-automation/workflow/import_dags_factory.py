...
import importlib
import golden_verification as _golden_verification
import import_automation_workflow as _import_automation_workflow

importlib.reload(_golden_verification)
importlib.reload(_import_automation_workflow)

from golden_verification import get_golden_test_imports
...
def find_catalog_file() -> str | None:
    ...
    candidates = [
        ...
        os.path.join(current_dir, "datacommons_airflow",
                     "imports_catalog.json"),
...
def load_catalog_and_register_dags(target_globals: dict[str, Any]) -> int:
    ...
    if not catalog_path:
        logging.info(
            "imports_catalog.json not found; dynamic DAG factory skipped.")
        return 0
    ...
    golden_allowlist = get_golden_test_imports()
    ...
    for entry in catalog:
        ...
        target_globals[dag_id] = build_dag(
            ...
            extra_tags=[
                "scheduled" if cron else "manual",
                entry.get("category", "data-commons")
            ],
            is_paused_upon_creation=True,
            golden_test_allowlist=golden_allowlist,
        )
        count += 1
