...
def build_catalog(
    ...
    for manifest_path in manifest_files:
        ...
        # Skip dated execution / output directories
        if re.search(r"\b\d{4}-\d{2}-\d{2}\b", rel_dir):
            continue
        ...
        for spec in import_specs:
            ...
            dag_id = import_name
            if allowlist_set and (dag_id not in allowlist_set and
                                  import_name not in allowlist_set):
                continue
            ...
            entry = {
                ...
                "import_name":
                    import_name,
                "full_import_name":
                    f"{rel_dir}:{import_name}",
                "script_dir":
                    rel_dir,
                "cron_schedule":
                    spec.get("cron_schedule"),
                "curator_emails":
                    spec.get("curator_emails", ["support@datacommons.org"]),
                "provenance_description":
                    spec.get("provenance_description", ""),
                "provenance_url":
                    spec.get("provenance_url", ""),
                "config_override":
                    spec.get("config_override", {}),
                "resource_limits":
                    spec.get("resource_limits", {}),
                "category":
                    category,
            }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compile manifest.json files into imports_catalog.json")
    parser.add_argument(
        "--data-dir",
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Optional list of specific DAG IDs / import names to include",
    )
    parser.add_argument(
        "--output",
