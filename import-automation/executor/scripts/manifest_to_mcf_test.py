...
"""
        ...
        self.assertIn("provenanceCategory: dcid:StatisticsProvenance",
                      output_mcf)
...
"""
        output_mcf = process_text(input_text,
                                  {"Gazetteer Files": "UsCensusBureau"})
        self.assertIn("Node: dcid:dc/s/UsCensusBureau", output_mcf)
        self.assertIn("Node: dcid:dc/d/UsCensusBureau_GazetteerFiles",
                      output_mcf)
        self.assertIn("isPartOf: dcid:dc/d/UsCensusBureau_GazetteerFiles",
                      output_mcf)
        self.assertIn("source: dcid:dc/s/UsCensusBureau", output_mcf)
        self.assertIn("Node: dcid:dc/s/UkGovernmentData", output_mcf)
        self.assertIn("Node: dcid:dc/d/UkGovernmentData_NationalStatistics",
                      output_mcf)
...
"""
        output_mcf = process_text(input_text)
        expected_hash = int(
            hashlib.sha256(b"custom_curator@example.com").hexdigest(),
            16) & 0xffffffff
