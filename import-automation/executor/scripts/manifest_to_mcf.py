...
def convert_to_mcf(import_proto, dataset_to_source, test_time=15700000):
    ...
    if not curator_email or curator_email == "imports@datacommons.org":
        ...
        curator_hash = int(
            hashlib.sha256(curator_email.encode('utf-8')).hexdigest(),
            16) & 0xffffffff
    ...
    source_id = None
    if dataset_name and dataset_name in dataset_to_source:
        source_id = dataset_to_source[dataset_name]
    else:
        if dataset_name:
            print(
                f"Warning: Source not found for dataset '{dataset_name}', skipping isPartOf and source"
            )

    if source_id:
        clean_dataset_name = clean_id(dataset_name)
        mcf_lines.append(
            f"isPartOf: dcid:dc/d/{source_id}_{clean_dataset_name}")
    ...
    if source_id:
        mcf_lines.append(f"source: dcid:dc/s/{source_id}")
...
def process_text(input_text, dataset_to_source=None):
    if dataset_to_source is None:
        dataset_to_source = {}
    ...
    try:
        ...
        if imports or dataset_sources:
            ...
            if imports:
                for imp in imports:
                    out_mcf.append(convert_to_mcf(imp, dataset_to_source))
...
def extract_dataset_source_map(textproto_path):
    dataset_to_source = {}
    with open(textproto_path, 'r') as f:
        content = f.read()
    if 'dataset_source' not in content:
        return dataset_to_source
    manifest = dc_manifest_pb2.DataCommonsManifest()
    try:
...
def build_dataset_to_source_map(directory):
    dataset_to_source = {}
    for root, _, files in os.walk(directory):
        for file in files:
            if file.endswith(".textproto"):
                path = os.path.join(root, file)
                dataset_to_source.update(extract_dataset_source_map(path))
    return dataset_to_source


def process_directory(path):
    dataset_to_source = build_dataset_to_source_map(path)
    for root, dirs, files in os.walk(path):
        for file in files:
            if file.endswith(".textproto"):
                textproto_path = os.path.join(root, file)
                mcf_path = os.path.join(root,
                                        file.replace(".textproto", ".mcf"))

                with open(textproto_path, 'r') as f:
                    input_text = f.read()

                try:
                    out_mcf = process_text(input_text, dataset_to_source)
