...
class ImportExecutorTest(unittest.TestCase):
    ...
    def test_create_venv(self):
        with tempfile.NamedTemporaryFile(mode='w+') as requirements:
            # beautifulsoup4 is not included in requirements.txt, so it
            # needs to be installed.
            # requests is included and does not need to be installed assuming
            # it has been installed on the host, since --system-site-packages
            # is set.
