...
_DEFAULT_PROJECT = os.environ.get(
    'GCS_PROJECT', 'datcom-import-automation-prod'
)
...
def get_simple_import_job_id(import_spec: dict, import_script: str) -> str:
  ...
  job_str = [
      import_script[import_script.find(_IMPORT_CONFIG_DIR) + 1 :]
      .replace(_DEFAULT_CONFIG_PREFIX, '')
      .removesuffix('.json')
  ]
  if import_name:
...
def cloud_run_simple_import_job(
  ...
  gcs_config = get_simple_import_gcs_config(
      config_file, import_name, version, copy_file=True
  )
  ...
  if not job:
    logging.error(f'Failed to setup cloud run job {job_id} for {config_file}')
    return None
