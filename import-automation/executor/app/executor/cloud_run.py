...
def create_or_update_cloud_run_job(
  ...
  Args:
    ...
    env_list: dict of environment variables as {'VAR': '<value>'}
  ...
  if not job:
    ...
    create_request = run_v2.CreateJobRequest(
        parent=parent, job_id=job_id, job=new_job
    )
    ...
    #new_job.template.template.containers.resources = (
    #    job.template.template.containers.resources
    #)
...
def execute_cloud_run_job(
  ...
  Returns:
    Jobs after execution is complete.
  """
  job_id = get_job_id(job_id)
  client = run_v2.JobsClient()
  parent = f"projects/{project_id}/locations/{location}"
