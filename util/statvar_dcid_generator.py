...
_QUANTITY_RANGE_REGEX_1 = re.compile(
    r'\[(?P<lower_limit>-|-?\d+(\.\d+)?) '
    r'(?P<upper_limit>-|-?\d+(\.\d+)?) '
    r'(?P<quantity>[A-Za-z_/\d]+)\]'
)
_QUANTITY_RANGE_REGEX_2 = re.compile(
    r'\[(?P<quantity>[A-Za-z_/\d]+) '
    r'(?P<lower_limit>-|-?\d+(\.\d+)?) '
    r'(?P<upper_limit>-|-?\d+(\.\d+)?)\]'
)
...
_PREPEND_APPEND_REPLACE_MAP = {
    'languageSpokenAtHome': {'append': 'SpokenAtHome'},
    'childSchoolEnrollment': {'prepend': 'Child'},
    'residenceType': {'prepend': 'ResidesIn'},
    'healthPrevented': {'prepend': 'Received'},
    'householderAge': {'prepend': 'HouseholderAge'},
    'householderRace': {'prepend': 'HouseholderRace'},
    'dateBuilt': {'append': 'Built'},
    'homeValue': {'prepend': 'HomeValue'},
    'numberOfRooms': {'prepend': 'WithTotal'},
    'isic': {'prepend': 'ISIC'},
    'establishmentOwnership': {'append': 'Establishment'},
    'householdSize': {'prepend': 'With'},
    'householdWorkerSize': {'prepend': 'With'},
    'numberOfVehicles': {'prepend': 'With'},
    'income': {'prepend': 'IncomeOf'},
    'grossRent': {'prepend': 'GrossRent'},
    'healthOutcome': {'prepend': 'With'},
    'healthPrevention': {'prepend': 'Received'},
    'propertyTax': {'prepend': 'YearlyTax'},
    'detailedLevelOfSchool': {'prepend': 'Detailed'},
    'medicalCondition': {'prepend': 'Condition'},
    'educationalAttainment': {'prepend': 'EducationalAttainment'},
    ...
    'usualHoursWorked': {'prepend': 'WorkPerWeek'},
    'workPeriod': {'prepend': 'WorkPerYear'},
    'dateOfEntry': {
        'prepend': 'DateOfEntry',
        'replace': 'Date',
        'replacement': '',
    },
    'placeOfBirth': {'prepend': 'PlaceOfBirth'},
    'dateMovedIn': {
        'prepend': 'MovedInDate',
        'replace': 'Date',
        'replacement': '',
    },
    'bachelorsDegreeMajor': {'prepend': 'BachelorOf'},
    'biasMotivation': {'prepend': 'BiasMotivation'},
    'offenderRace': {'prepend': 'OffenderRace'},
    'offenderEthnicity': {'prepend': 'OffenderEthnicity'},
    'locationOfCrime': {'prepend': 'LocationOfCrime'},
    'victimType': {'prepend': 'VictimType'},
    'mothersRace': {'prepend': 'Mother'},
    'mothersEthnicity': {'prepend': 'Mother'},
    'mothersNativity': {'prepend': 'Mother'},
    'mothersMaritalStatus': {'prepend': 'Mother'},
    'mothersAge': {'prepend': 'Mother'},
    'mothersEducation': {'prepend': 'Mother'},
    'importSource': {
        'prepend': 'ImportFrom',
    },
    'exportDestination': {
...
def _capitalize_process(word: str) -> str:
  ...
  if word:
    ...
    for tok in word.split('/'):
      if tok:
        if tok[0].isdigit() and len(words) > 0 and words[-1][-1].isdigit():
          words.append('-')
...
def _prepend_append_replace(
    word, prepend='', append='', replace='', replacement=''
):
...
def _process_constraint_property(prop: str, value: str) -> str:
  ...
  if prop in _PREPEND_APPEND_REPLACE_MAP:
    name = _prepend_append_replace(name, **_PREPEND_APPEND_REPLACE_MAP[prop])
